...
const state = vi.hoisted(() => ({ cookies: {} }));
const db = vi.hoisted(() => ({
  upsert: vi.fn(),
  findMany: vi.fn(),
}));
vi.mock("next/headers", () => ({
  cookies: async () => ({
    get: (name) =>
      state.cookies[name] != null
        ? { name, value: state.cookies[name] }
        : undefined,
  }),
...
beforeEach(async () => {
  vi.resetModules();
  state.cookies = {};
  db.upsert.mockReset().mockImplementation(async (args) => args.create);
...
describe("LEGACY MODE (no AUTH_SECRET) - the backward-compatibility contract", () => {
  it("writes with no seat at all, stamped with the lens, exactly as before", async () => {
    ...
    expect(db.upsert.mock.calls[0][0].where.transactionId_ownerId.ownerId).toBe(
      ME,
    );
  });
...
describe("MULTI-USER MODE - the forged-cookie hole, closed", () => {
  ...
  it("stamps the SEAT, not the lens - flipping the lens no longer changes the author", async () => {
    ...
    expect(
      db.upsert.mock.calls[0][0].where.transactionId_ownerId.ownerId,
    ).not.toBe(RIVAL);
  });
...
describe("WRITE COST - the note is visible without rebuilding the corpus", () => {
  ...
  const FRESH = {
    transactionId: "fx-2025-pivot",
    reasoning: "why I pivoted",
    posture: null,
  };
  async function warm() {
    process.env.DATABASE_URL = "postgresql://example/db";
  ...
  it("a successful write is visible to the next read with NO reassembly", async () => {
    ...
    db.upsert.mockImplementationOnce(async (args) => ({
      ...args.create,
      createdAt: new Date(),
      updatedAt: new Date(),
    }));
  ...
  it("overwrites an existing note in place rather than stacking a second one", async () => {
    ...
    await POST(post(NOTE));
    const after = await history.getLeagueHistory();
    expect(history.myAnnotation(after, NOTE.transactionId)?.reasoning).toBe(
      "because",
    );
