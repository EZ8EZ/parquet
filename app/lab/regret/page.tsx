...
function WeeklyBars({ weeks }: { weeks: WeekRow[] }) {
  ...
  return (
    ...
    >
      <line x1={padX} y1={base} x2={W - padX} y2={base} stroke="var(--color-border)" strokeWidth={1} />
      {weeks.map((w, i) => {
        ...
        return (
          <g key={w.week}>
            ...
            />
            {w.emptySlots > 0 && (
              <circle cx={x + Math.round(barW / 2)} cy={base + 6} r={2} fill="var(--color-warn)" />
            )}
            {(i === 0 || (i + 1) % 5 === 0) && (
              <text
...
function WidestWeek({ w }: { w: WeekRow }) {
  return (
    <Card>
...
function SeasonPills({ seasons, active }: { seasons: string[]; active: string }) {
  return (
...
function Headline({ l }: { l: RegretLedger }) {
  const share = l.bestTotal > 0 ? Math.round((l.bankedTotal / l.bestTotal) * 100) : 0;
  ...
  return (
    <div className="grid grid-cols-3 gap-2">
      <Stat label="Banked" value={whole(l.bankedTotal)} sub={`${share}% of available`} />
      <Stat label="Available" value={whole(l.bestTotal)} sub={`${l.slotsTotal} slots`} />
      <Stat
        label="On the table"
...
export default async function RegretPage({
  ...
  if (chosen.lastScoredWeek === 0) {
    return (
      <div>
        {header}
        <SeasonPills seasons={options.map((o) => o.season)} active={chosen.season} />
        <EmptyState title={`${chosen.season} has not been played`}>
          The league reports no scored weeks for this season, so there are no lineups to
          read. Nothing here is estimated to fill the gap.
        </EmptyState>
      </div>
    );
  }
  ...
  return (
    <div>
      {header}
      <SeasonPills seasons={played.map((o) => o.season)} active={chosen.season} />

      <Card className="mb-3">
        <p className="text-body leading-relaxed text-ink">
          Best in hindsight is not best decision. A manager who banked 28 on Tuesday
          could not know Thursday would bring 41.
        </p>
        <p className="mt-1.5 text-body leading-relaxed text-muted">
          This is a record of how the weeks turned out, not a reading of how they were
          chosen. Nothing below is a mistake.
        </p>
      </Card>
      ...
      {ledger.emptySlots > 0 && (
        <Card className="mt-2 border-warn/30 bg-warn/5">
          <p className="text-body leading-relaxed text-ink">
            ...
            filled, and {ledger.zeroSlots} more banked a name that scored nothing.
          </p>
          <p className="mt-1.5 text-meta leading-snug text-muted">
            This is the one line here that is about you rather than about the week: an
            empty slot needed no foresight to avoid. It still cannot tell a forgotten
            slot from a deliberate one, and a team playing for draft position leaves the
            same trace.
          </p>
        </Card>
      )}
      ...
      <p className="mb-2 mt-1 text-micro text-faint">
        Solid is banked, faint is what the roster produced. A gold dot marks a week with
        an unfilled slot.
      </p>
        ...
        <Disclosure summary="How this is built, and what it cannot see">
          <ul className="space-y-1.5">
            <li>
              ...
              <code className="font-mono">starters_points</code>. Each one was checked
              back against the box scores under this league&apos;s scoring settings:{" "}
              {ledger.verifiedSlots} of {ledger.filledSlots} filled slots matched a real
              game. Anything that did not match is left in the record and flagged rather
              than adjusted.
            </li>
            <li>
              Available means the best single game each rostered player produced that
              week, seven distinct players, one game each. That is how the format works:
              a slot holds one player-game and a player can fill at most one slot.
            </li>
            <li>
              Position eligibility is NOT applied. Sleeper reports only today&apos;s
              eligibility, and 193 of the 2,244 filled slots this league actually played
              in 2025 would be illegal under it. Grading real lineups against a rulebook
              they did not play under would be worse than the upper bound this produces,
              so &quot;available&quot; is an upper bound and is stated as one.
            </li>
            <li>
              IR and taxi players sit in the weekly roster list and could not legally
              have been started. Per-week reserve status is not recoverable from any
              endpoint, so a little of the pool was never actually startable.
            </li>
            <li>
              {ledger.slotsAtPlayerBest} of {ledger.filledSlots} filled slots banked
              that player&apos;s own best game of the week. A fact about the week, not a
              score.
            </li>
            <li>
              Read on demand: {weeksRead} lineup requests and {playersFetched} player
              stat requests for this season, cached for 30 minutes and never touched by
              the rest of the app.
            </li>
          </ul>
        </Disclosure>
