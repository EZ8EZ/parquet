 ...
 */
function SplitBars({ rows }: { rows: { label: string; players: number; picks: number }[] }) {
  const W = 320;
  const X = 4;
  const TRACK = W - X * 2;
...
function Column({
  ...
  tci: number;
  duration: number;
  /** Draws the card dashed. Deliberately NOT tinted with the accent: gold reads as
   *  "the good one" in this design system, and neither column is being preferred. */
...
function Gaps({
  ...
  if (c.draftless) {
    lines.push("No draft data is available, so only the waiver record is reconstructed.");
  }
  return (
  ...
  if (rosterId == null) {
    return (
      <div>
        ...
        <Card>
          <p className="text-body text-muted">No roster to read. Pick a team first.</p>
        </Card>
      </div>
    );
  }
  ...
  return (
    <div>
      ...
      <p className="mt-2 text-meta leading-snug text-secondary">
        ...
        strategy, and the two columns are the argument, not the answer.
      </p>
    </div>
  );
}
