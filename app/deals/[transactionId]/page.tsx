...
import type { ManagerMetric, PlayerNow } from "@/lib/tradegraph";
...
export default async function DealPage({
  ...
  const nowOf = (pid: string): PlayerNow | undefined => {
          ...
          <div className="flex items-start justify-between gap-2">
            {s.manager ? (
              <ManagerLink
                node={s.manager}
                metric={
                  s.manager.isFormer ? undefined : managerMetrics[s.manager.rosterId]
                }
                isMe={s.manager.isMe}
              />
            ) : (
      ...
      {anyValued && (
        <>
          ...
          <Card>
            <SideBars
              data={sides.map((s) => ({
                label: s.manager?.name ?? s.side?.name ?? `Roster ${s.rosterId}`,
            ...
            <p className="mt-2 text-meta leading-snug text-muted">
              Today&apos;s prices, on the players only. Not a grade and not a winner.
            </p>
            <Disclosure summary="What this number cannot tell you" className="mt-1">
              <p>
                This is hindsight. It prices what each side received at what those
                players are worth NOW, which is the right way to read an outcome and
                the wrong way to read a decision - nobody trading in {record.season}
                {" "}
                knew any of it. The app holds no historical ranking snapshots, so a
                value-at-the-time version is not available and this one says today, and
                only today.
              </p>
              <p className="mt-1.5">
                Players only. Commissioner-executed trades reach us with no picks
                attached at all, so counting the picks that happen to be recorded would
                make a number that looks complete and is not. The direction of the bias
                is worth knowing: a side that sent picks and took back players looks
                better here than it was, and a side that sold for picks looks worse.
                What the picks became is listed above, unpriced.
              </p>
            </Disclosure>
          </Card>
        </>
