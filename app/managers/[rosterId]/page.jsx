...
export default async function ManagerDetailPage({ params }) {
  ...
  const titles = p.userId
    ? titleSummariesByOwner(h, principals).get(p.userId)
    : undefined;
  ...
  return (
    <div>
      ...
      {buybacks.length > 0 && (
        <>
          ...
          <p className="mt-1.5 text-meta leading-snug text-secondary">
            A pick that came home. It says what happened, not why: the app
            cannot see intent, and a pick can return as a throw-in as easily as
            on purpose.
          </p>
        </>
      )}
      ...
      {luck && luck.gamesPlayed > 0 && (
        <>
          ...
          <p className="mt-1.5 text-note leading-snug text-muted">
            ...
            of {luck.luckWins >= 0 ? "cushion" : "drag"} versus what their
            scoring alone earned them
            {luck.luckiest &&
            luck.unluckiest &&
            luck.luckiest !== luck.unluckiest
          ...
          {!luck.allPlay && (
            <p className="mt-1 text-meta leading-relaxed text-secondary">
              ...
              loaded for the live provider (see lib/history.ts). Not the same as
              a true all-play record, but the same honest question: does the
              record match the scoring.
            </p>
          )}
        </>
      )}
      ...
      <SectionHeader
        title={
          isMe ? "Your own file" : `More on ${p.teamName ?? p.displayName}`
        }
      />
      <ManagerRail
        rosterId={rosterId}
      ...
      <p className="mt-3 text-meta leading-relaxed text-secondary">
        ...
        net picks). Behavior, not stated intent. The window line is the one
        reading here that comes from what they hold rather than from what they
        have done.
      </p>
    </div>
  );
