...
const state = vi.hoisted(() => ({
  seat: { enforced: true, ownerId: "u1" },
  owner: "u1",
  origin: "https://parquet.example",
}));
...
describe("SeatLinks", () => {
  ...
  it.each([null, ""])(
    "renders NO link at all when the origin is %o",
    async (origin) => {
      state.origin = origin;
      const html = await render(base);
  ...
  it("SKIPS an unsignable owner id instead of throwing the whole page away", async () => {
    ...
    expect([...html.matchAll(CLAIM_LINK)]).toHaveLength(
      base.rosters.length - 1,
    );
