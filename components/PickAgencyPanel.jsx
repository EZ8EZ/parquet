...
const BAR_W = 300;
const BAR_H = 14;
function SplitBar({ controlled, total }) {
  ...
  return (
    ...
    >
      ...
      />
      {w > 0 && (
        <rect x={0} y={0} width={w} height={BAR_H} rx={3} fill={CHART_ACCENT} />
      )}
    </svg>
  );
}
export function PickAgencyPanel({ reads, summary, orderNote }) {
        ...
        {settledCount > 0 && (
          <>
            ...
            <p className="text-meta leading-snug text-secondary">
              ...
              {settledCount === 1 ? "was" : "were"} set by a season that is
              already over, so{" "}
              {settledCount === 1 ? "its slot is" : "their slots are"} no longer
              anybody&apos;s to move.{" "}
              {live.length > 0
                    ...
                    >
                      <span className="min-w-0">
                        ...
                        <span className="block text-secondary">
                          ...
                          <span className="figure">{r.determiningSeason}</span>{" "}
                          season sets it
                        </span>
                      </span>
