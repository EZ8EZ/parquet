 ...
 */
const POSTURE_GLYPH: Record<string, ComponentType<{ size?: number; className?: string; "aria-hidden"?: boolean }>> = {
  contending: Circle,
  ascending: Triangle,
  rebuilding: Square,
  straddling: Diamond,
...
export function PostureGlyph({ posture, className }: { posture: string | null | undefined; className?: string }) {
  const Icon = posture ? POSTURE_GLYPH[posture] : undefined;
  if (!Icon) return null;
  return (
    <Icon size={9} aria-hidden={true} className={cn("inline-block shrink-0 align-baseline", className)} />
  );
}
