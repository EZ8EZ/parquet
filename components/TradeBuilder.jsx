...
function AssetRow({ label, meta, value, onRemove, player }) {
  return (
...
export function TradeBuilder({
  ...
  const [initial] = useState(() => {
    ...
    const players = (list) =>
      list.map((id) => playerById.get(id)).filter((p) => !!p);
    const picks = (list) =>
      list.map((id) => pickById.get(id)).filter((p) => !!p);
    return {
  ...
  const [give, setGive] = useState(initial.give);
  const [get, setGet] = useState(initial.get);
  const [givePicks, setGivePicks] = useState(initial.givePicks);
  ...
  const giveTotal =
    give.reduce((s, p) => s + p.value, 0) +
    givePicks.reduce((s, p) => s + p.value, 0);
  const getTotal =
    get.reduce((s, p) => s + p.value, 0) +
  ...
  async function evaluate() {
    ...
    try {
      ...
      const res = await fetch("/api/trade", {
        ...
        body: JSON.stringify({
          give: {
            playerIds: give.map((p) => p.id),
            picks: pickBody(givePicks),
          },
  ...
  const modal = useMemo(() => {
    ...
    return {
      ...
      emptyText: giving ? "You own no tradeable picks." : "No picks match.",
      items: source
        .filter((o) => !chosen.has(o.id))
        .map((o) => ({
          id: o.id,
    ...
    <div>
      <div className="grid grid-cols-2 gap-2">
        <SideColumn
          ...
          onRemovePick={(id) =>
            setGivePicks((x) => x.filter((p) => p.id !== id))
          }
        />
        <SideColumn
          label="You get"
          ...
          onRemovePick={(id) =>
            setGetPicks((x) => x.filter((p) => p.id !== id))
          }
        />
      </div>
 ...
 */
function counterpartyOf(incoming) {
  const byRoster = new Map();
  for (const a of incoming) {
    if (a.ownerRosterId == null) continue;
    const cur = byRoster.get(a.ownerRosterId) ?? {
      value: 0,
      name: a.owner ?? "",
    };
    byRoster.set(a.ownerRosterId, {
      value: cur.value + a.value,
      name: cur.name || a.owner || "",
    });
  }
  let best = null;
  let bestValue = -1;
...
function TradeResult({ r, leagueId, counterparty }) {
  ...
  return (
    ...
    <div className="receipt mt-4 space-y-2">
      ...
      <Block title="The assumption that has to be true" tone="accent">
        {r.keyAssumption}
      </Block>
      <Block title="What your history says" tone="warn">
        {r.historyCheck}
      </Block>
      {r.consolidationNote && (
      ...
      {counterparty && (
        <div className="rounded-[--radius] border border-border bg-surface p-3">
          ...
          <p className="text-body leading-relaxed text-ink/90">
            You are asking{" "}
            <span className="font-semibold text-ink">{counterparty.name}</span>{" "}
            to say yes. Read what they have actually paid for in five seasons,
            and what the app would have proposed to them on its own.
          </p>
          <ManagerRail
...
function Block({
  ...
  return (
    <div className={cn("rounded-[--radius] border bg-surface p-3", border)}>
      <div
        className={cn(
          "mb-1 text-meta font-semibold uppercase tracking-wide",
          head,
        )}
      >
        {title}
