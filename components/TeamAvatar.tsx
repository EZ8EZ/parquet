...
export function TeamAvatar({
  ...
  /** Team name if set, else the manager's display name. */
  name: string;
  ...
  if (avatarId) candidates.push(`https://sleepercdn.com/avatars/thumbs/${avatarId}`);
  ...
  return (
    <span
      ...
      style={{ width: px, height: px, fontSize: Math.round(px * 0.36), background: bg }}
    >
      {initials(name)}
    </span>
  );
