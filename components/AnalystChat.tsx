        ...
        <div className="space-y-4">
          <div className="rounded-[--radius] border border-border bg-surface/60 p-4">
            <div className="mb-2 flex items-center gap-2 text-negative">
              <ShieldAlert size={16} />
              <span className="text-xs font-semibold uppercase tracking-wide">Adversarial by design</span>
            </div>
          ...
          <div className="space-y-2">
            {SUGGESTED.map((s) => (
              <button
                key={s}
                onClick={() => ask(s)}
                className="block w-full rounded-[--radius-sm] border border-border bg-surface/60 px-3.5 py-3 text-left text-sm text-ink transition-colors hover:border-accent"
              >
                {s}
              </button>
            ))}
        ...
        <div className="space-y-3">
          {messages.map((m, i) => (
            <div
              key={i}
              className={cn(
                "rounded-[--radius] p-3.5 text-sm leading-relaxed",
                m.role === "user"
          ...
          {loading && (
            <div className="mr-2 flex items-center gap-2 rounded-[--radius] border border-border bg-surface/70 p-3.5 text-sm text-muted">
      ...
      >
        ...
        >
          <textarea
            ...
            className="max-h-32 flex-1 resize-none rounded-[--radius] border border-border bg-surface px-3 py-2.5 text-sm text-ink placeholder:text-faint focus:border-accent focus:outline-none"
          />
          <button
            type="submit"
