 ...
 */
export function layoutRows(times, floors) {
  const n = times.length;
  const floorAt = (i) => Math.max(MIN_ROW, floors?.[i] ?? MIN_ROW);
  ...
  for (let i = 0; i < n; i++) {
    const raw =
      span > 0
        ? Math.round((budget * (times[i] - first)) / span)
        : i * floorAt(i - 1);
 ...
 */
function isHomecoming(h) {
  const pick = parsePickKey(h.assetKey);
  return (
    pick != null &&
    pick.originalRoster === h.to &&
