...
export async function collectTradedPicks(provider, chain) {
  // Fanned out for the same reason collectTransactions below is: each season's
  // fetch is independent of every other season's, so issuing them concurrently
  // bounds the group by the slowest single request instead of the sum of all of
  // them. The dedup pass afterward still walks `chain` in its ORIGINAL order (not
  // resolution order), so "keep the first (oldest league) occurrence" means exactly
  // what it always meant - only the fetching moved, not the merge semantics.
