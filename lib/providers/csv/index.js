...
/** A CSV cell that is absent, blank or not a number reads as "not stated", never NaN. */
...
export class CsvProvider {
  ...
  async getPlayers() {
    return this.rows("players").map((p) => ({
      ...
      injuryBodyPart: null,
      injuryNotes: null,
      // Both optional columns, and both read rather than nulled: a CSV export of a
      // Sleeper payload already carries them, and a hand-built CSV that omits them
      // gets the same honest "no depth chart entry" the live data gives 119 of 593
      // players (see lib/depth).
      depthChartPosition: p.depth_chart_position
        ? p.depth_chart_position.trim().toUpperCase()
        : null,
      depthChartOrder: intOrNull(p.depth_chart_order),
      newsUpdated: intOrNull(p.news_updated),
      status: "ACT",
      number: null,
