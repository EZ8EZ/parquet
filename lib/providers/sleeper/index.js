...
async function getJson(path, schema, { retries = 3, noStore = false } = {}) {
  let lastErr;
  ...
  throw new Error(
    `Sleeper request failed after ${retries + 1} attempts: ${path}: ${String(lastErr)}`,
  );
}
...
export class SleeperProvider {
  ...
  async getUser(username) {
    return toUser(
      await getJson(`/user/${encodeURIComponent(username)}`, RawUser),
    );
  }
  async getLeagues(userId, sport, season) {
  ...
  async getTransactionsForSeason(leagueId, week, season) {
    const raw = await getJson(
      `/league/${leagueId}/transactions/${week}`,
      RawTransactionArr,
    );
  ...
  async getBracket(leagueId, kind) {
    const raw = await getJson(
  ...
  async getDraftPicks(draftId) {
    return memo(draftPicksCache, draftId, DRAFTS_TTL_MS, async () => {
      const raw = await getJson(`/draft/${draftId}/picks`, RawMadeDraftPickArr);
 ...
 * lib/ingest.ts).
 */
async function memo(cache, key, ttlMs, load) {
