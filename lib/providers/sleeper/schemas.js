...
export const RawPlayer = z.object({
  ...
  // BOTH HALVES OF THE DEPTH CHART, and it took both to make either usable. This
  // schema parsed `depth_chart_order` alone for months, which is a number with no
  // referent: "order 2" is 2nd at SOMETHING, and on a live NBA roster the same
  // integer appears at PG, at SG and at C on the same team in the same payload.
  // `depth_chart_position` is the position that number is an order WITHIN.
  //
  // A string, not an enum: it is PG/SG/SF/PF/C on live NBA data, but an enum here
  // would turn one unfamiliar code into a failed parse for the whole 593-player
  // payload, which is exactly the trade this file's header refuses to make.
  depth_chart_position: str.nullish(),
  depth_chart_order: num.nullish(),
  // The only freshness signal Sleeper attaches to a player record (ms epoch). It is
  // NOT a depth-chart timestamp - it moves when anything about the player's news
  // moves - so it can only ever support "the record was touched then", never "the
  // chart was correct then". Mapped because a fact shown without its age invites
  // being read as current, and this is the honest ceiling on what we can say.
  news_updated: num.nullish(),
  status: str.nullish(),
  number: num.nullish(),
...
export function toPlayer(r) {
  ...
  return {
    ...
    // Normalised here, once, so no consumer has to guess whether it is comparing
    // "PG" with "pg": the raw payload is consistent today, and one lowercase code
    // would otherwise split a position group in two on the surface that reads it.
    depthChartPosition: r.depth_chart_position
      ? r.depth_chart_position.trim().toUpperCase()
      : null,
    depthChartOrder: r.depth_chart_order ?? null,
    newsUpdated: r.news_updated ?? null,
