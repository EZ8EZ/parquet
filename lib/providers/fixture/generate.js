import { CURATED, MANAGERS, fillerPlayers } from "./data.js";
import { NBA_TEAM_NAMES } from "../../depth/teams.js";
import { CHART_POSITIONS } from "../../depth/index.js";
...
export function leagueIdFor(season) {
  return `fx-nba-${season}`;
}
function prevLeagueId(season) {
...
// ---------- Players ----------
/**
 * THE FIXTURE GETS REAL NBA TEAMS AND A REAL-SHAPED DEPTH CHART.
 *
 * Fixture players carried `team: null` and `depthChartOrder: null` from the start,
 * which was fine while nothing read either. `/depth/[team]` reads both, and a fixture
 * that answers "no team, no chart" for all 290 players would leave the one surface
 * that consumes depth data untestable end-to-end and invisible in `pnpm dev` - the
 * fixture exists precisely so that is never the trade.
 *
 * The DEFECTS ARE THE POINT. This does not lay out tidy 1-2-3 charts, because live
 * Sleeper data never does (measured 2026-08-19: 116 of 149 groups non-contiguous, 43
 * with a duplicate order, 18 with no order 1 at all, 119 of 593 players with no entry
 * whatsoever, a quarter of charted players charted away from their listed position).
 * A fixture that only produced the tidy case would make the surface look correct while
 * hiding every shape it was built to handle. So the pattern below reproduces all five,
 * deterministically, with no RNG: same seed data, same chart, every run.
 */
const NBA_TEAMS = Object.keys(NBA_TEAM_NAMES);
/** Non-contiguous by construction: the 3rd player at a position is a 5, not a 3. */
const ORDER_PATTERN = [1, 2, 5, 7, 8];
/**
 * @param {{playerId: string, position: string|null, depthChartPosition: string|null,
 *   depthChartOrder: number|null, team: string|null, newsUpdated: number|null}[]} players
 */
function assignDepthCharts(players) {
  /** @type {Map<string, number>} */
  const filled = new Map();
  /** @type {Map<string, number>} */
  const seen = new Map();
  players.forEach((p, i) => {
    const team = NBA_TEAMS[i % NBA_TEAMS.length];
    p.team = team;
    // COUNTED PER TEAM, not off the payload index, and that is a bug fix rather than
    // a style choice: teams are dealt round-robin over thirty, so any rule of the
    // form `i % 5` is perfectly correlated with `i % 30` (5 divides 30) and the first
    // version of this gave six teams a chart of zero players and twenty-four a chart
    // with no gaps at all. Per-team counters cannot correlate with the team.
    const nth = seen.get(team) ?? 0;
    seen.set(team, nth + 1);
    // Every fifth player on a team has no depth chart entry at all, matching the
    // 119-of-593 share the live payload carries. These are the players whose surface
    // shows the honest empty state.
    if (nth % 5 === 3) {
      p.depthChartPosition = null;
      p.depthChartOrder = null;
      p.newsUpdated = null;
      return;
    }
    // A quarter of live charted players are charted away from their listed position
    // (Bronny James: listed SG, charted PG), so a quarter of these are too.
    const shifted =
      nth % 4 === 2
        ? CHART_POSITIONS[(CHART_POSITIONS.indexOf(p.position ?? "PG") + 1) % 5]
        : p.position;
    const pos = shifted ?? "PG";
    const key = `${team}|${pos}`;
    const inGroup = filled.get(key) ?? 0;
    filled.set(key, inGroup + 1);
    p.depthChartPosition = pos;
    // The duplicate case, and it has to be reachable by a reader: the third player
    // into a group repeats the previous order instead of advancing, so those groups
    // list two players level - exactly what 43 of the live 149 groups do.
    const step = inGroup === 2 ? 1 : inGroup;
    p.depthChartOrder = ORDER_PATTERN[step % ORDER_PATTERN.length];
    // A record age, spread across roughly the same range the live payload has (hours
    // old at the freshest, years at the stalest), so the freshness line has something
    // real to say. Fixed epoch, not `Date.now()`, so the fixture stays deterministic.
    p.newsUpdated = FIXTURE_NEWS_EPOCH - i * 36 * 60 * 60 * 1000;
  });
}
/** 2026-08-01T00:00:00Z. A constant, because a fixture that drifts is not a fixture. */
const FIXTURE_NEWS_EPOCH = 1785110400000;
function buildPlayers() {
  const base = [...CURATED, ...fillerPlayers(190, CURATED.length)];
  const players = [];
  ...
  base.forEach((p, i) => {
    ...
    players.push({
      ...
      injuryBodyPart: null,
      injuryNotes: null,
      // Overwritten wholesale by `assignDepthCharts` below - declared here only so
      // the shape matches `Player` before it runs.
...
function ownerIdForSeason(rosterId, season, users) {
 ...
 * anyway.
 */
function buildBracket(rosters, seasonIndex) {
  const seeds = [...rosters]
    .sort(
      (a, b) =>
  ...
  const game = (matchId, round, t1, t2, winner, placement = null) => ({
    matchId,
    round,
    placement,
    team1: t1,
 ...
 */
const DRAFT_SEASONS = ["2023", "2024", "2025", "2026"];
const DRAFT_ROUNDS = 3;
export function draftIdFor(season) {
...
export function generateCorpus() {
  ...
  const playerVal = (pid) => Math.max(1, 320 - (rankOf.get(pid) ?? 300));
  // Snake draft top 224 players across 14 teams.
  const rosters = Array.from({ length: N_TEAMS }, (_, i) => ({
    rosterId: i + 1,
    players: new Set(),
  }));
  ...
  const freeAgents = new Set(ordered.slice(di).map((p) => p.playerId));
  ...
  const worstNonProtected = (roster, exclude) =>
  ...
  function place(playerId, target) {
  ...
  const findPick = (season, round, original) =>
    picks.find(
      (p) =>
        p.season === season &&
        p.round === round &&
    ...
    for (let w = 1; w <= weeks; w++) {
      ...
      for (let i = 0; i < N_TEAMS; i += 2) {
        ...
        mlist.push({
          rosterId: ra,
          matchupId,
          points: Math.round(pa * 10) / 10,
          week: w,
        });
        mlist.push({
          rosterId: rb,
          matchupId,
          points: Math.round(pb * 10) / 10,
          week: w,
        });
  ...
  const ownerUserId = (rosterId, season) =>
  ...
  const rookiePool = ordered.slice(
    Math.max(0, ordered.length - 42 * DRAFT_ROUNDS),
  );
  const completeDraftSeasons = DRAFT_SEASONS.filter(
    (s) => s !== CURRENT_SEASON,
  );
  for (const season of DRAFT_SEASONS) {
    ...
    const prior = SEASONS[SEASONS.indexOf(season) - 1];
  ...
  function buildArchetypeTrade(arch, r, c, season, week) {
    if (arch === "hoarder") {
...
function archetypeConfig(arch) {
  switch (arch) {
    case "churner":
      return { trades: 4, waivers: 3 };
    case "hoarder":
      return { trades: 2, waivers: 1 };
    case "ghost":
      return { trades: 0, waivers: 0 };
    case "panic":
      return { trades: 3, waivers: 2 };
    case "name-chaser":
      return { trades: 2, waivers: 2 };
    case "streamer":
      return { trades: 1, waivers: 9 };
    case "balanced":
      return { trades: 2, waivers: 4 };
    default:
      return { trades: 1, waivers: 2 };
  }
}
function toDomainRoster(r, season, wins, losses, fpts, users) {
