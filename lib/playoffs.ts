...
/** Final places for one season, plus who holds the title. */
export interface SeasonResult {
  season: string;
  /** rosterId -> final place. Only teams a placement game decided, so 1..8 here. */
  placeByRoster: Map<number, number>;
  championRosterId: number | null;
  runnerUpRosterId: number | null;
  /** How many places the bracket actually decided. */
  placesDecided: number;
}
 ...
 */
export function placementsFrom(games: readonly BracketGame[]): Map<number, number> {
...
export function championFrom(games: readonly BracketGame[]): number | null {
  for (const g of games) {
...
export function seasonResult(
  ...
  for (const [rosterId, place] of placeByRoster) if (place === 2) runnerUp = rosterId;
  return {
    season,
    placeByRoster,
...
export function seasonResults(h: LeagueHistory): SeasonResult[] {
...
export interface Title {
  season: string;
  rosterId: number;
  /** The person who held that roster THAT season, not whoever holds it now. */
  ownerId: string | null;
}
 ...
 */
export function titles(h: LeagueHistory, principals: PrincipalIndex): Title[] {
 ...
 */
export function playoffPlaces(h: LeagueHistory): Map<number, number> | null {
