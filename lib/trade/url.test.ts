...
describe("parseTradeParams / tradeQueryString round trip", () => {
  ...
  it("round-trips a package with only one side populated", () => {
    const pkg: TradePackageIds = { give: ["p1", "p2"], get: [], givePicks: [], getPicks: [] };
...
describe("isEmptyTradePackage", () => {
  it("is true only when every side is empty", () => {
    ...
    expect(isEmptyTradePackage({ give: ["p1"], get: [], givePicks: [], getPicks: [] })).toBe(
      false,
    );
    expect(
      isEmptyTradePackage({ give: [], get: [], givePicks: ["2027-1-1"], getPicks: [] }),
    ).toBe(false);
  });
});
