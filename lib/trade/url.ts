...
export interface TradePackageIds {
  give: string[];
  get: string[];
  givePicks: string[];
  getPicks: string[];
...
/**
 * The read side of `URLSearchParams` / Next's `ReadonlyURLSearchParams`, stated
...
export const EMPTY_TRADE_PACKAGE: TradePackageIds = {
  give: [],
  get: [],
  givePicks: [],
  getPicks: [],
...
function encodeList(ids: readonly string[]): string {
  return ids
    .map((id) => id.trim())
    .filter(Boolean)
    .join(",");
 ...
 */
export function parseTradeParams(params: ParamReader): TradePackageIds {
  return {
...
export function tradeQueryString(pkg: TradePackageIds): string {
...
export function tradeHref(pkg: TradePackageIds): string {
  return `/trade${tradeQueryString(pkg)}`;
}

export function isEmptyTradePackage(pkg: TradePackageIds): boolean {
  return (
    pkg.give.length === 0 &&
    pkg.get.length === 0 &&
