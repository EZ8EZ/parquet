...
/**
 * ONE PLAYER'S PAGE ON SLEEPER.
 * Route: `/nba/players/:playerId` — the ONE route in this file that a status code can
 * actually verify, because it is genuinely server-rendered rather than the SPA shell
 * (see the header: a real id returns the player's name in the `<title>`, a nonsense id
 * returns a nameless one, and the sport comes FIRST in the path).
 *
 * It exists for the depth chart (`/depth/[team]`), whose whole content is Sleeper's own
 * published chart: "this came from Sleeper" is worth more as a link to the exact screen
 * it came from than as a sentence. `isNumericId` keeps the fixture provider's `p12`
 * ids from producing a dead link - `<OpenInSleeper>` renders nothing for a null href.
 */
export function sleeperPlayerUrl(playerId) {
