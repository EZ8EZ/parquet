...
const LIVE_BODY_PARTS = [
  "Knee", "Ankle", "Rest", "Hamstring", "Foot", "Finger", "Toe", "Shoulder",
  "Achilles", "Calf", "Undisclosed", "Wrist", "Back", "Hip", "Quadriceps",
  "Ribs", "Elbow", "Eye", "Illness", "Thumb", "Nose", "Heel", "Groin",
  "Lower Leg", "Not Injury Related",
];
const LIVE_NOTES = [
  "Surgery", "Sprain", "Strain", "Soreness", "Fracture", "Bruise", "Inflammation",
];
const LIVE_STATUSES = ["DTD", "Out", "IR"];

/** Real rows from the live feed, used as calibration fixtures. */
const HALIBURTON = {
  status: "DTD", bodyPart: "Achilles", notes: "Surgery", age: 26,
};
const LILLARD = {
  status: "DTD", bodyPart: "Achilles", notes: "Surgery", age: 36,
};
const GIANNIS = {
  status: "DTD", bodyPart: "Knee", notes: "Bruise", age: 31,
};
const DAVIS_FINGER = {
  status: "DTD", bodyPart: "Finger", notes: "Sprain", age: 33,
};
...
const TOPIC_BACK = {
  status: "DTD", bodyPart: "Back", notes: "Surgery", age: 20,
};
...
describe("the live Sleeper NBA vocabulary is fully mapped", () => {
  ...
  it("has a penalty and a slope for every class it can produce", () => {
    const classes = new Set(Object.values(VALUATION_CONFIG.injury.bodyPartClass));
  ...
  it("falls back to the unknown class rather than throwing on a new body part", () => {
    ...
    expect(injuryMultiplier({ status: "DTD", bodyPart: "Spleen" })).toBeLessThan(1);
  });
});
...
describe("age changes what an injury means", () => {
  ...
  it("makes Lillard at 36 a far heavier write-down than Haliburton at 26", () => {
    expect(injuryMultiplier(LILLARD)).toBeLessThan(injuryMultiplier(HALIBURTON) - 0.2);
  });

  it("climbs soft-tissue recurrence risk steeply with age", () => {
    const ham = (age: number) =>
      injuryMultiplier({ status: "DTD", bodyPart: "Hamstring", notes: "Strain", age });
  ...
  it("bounds the age scale at both ends so no term can run away", () => {
    ...
    expect(injuryAgeScale(VALUATION_CONFIG.injury.ageReference, "achilles")).toBe(1);
  });

  it("uses the league's median age when a flagged player has no age on file", () => {
    expect(injuryMultiplier({ ...HALIBURTON, age: null })).toBe(
      injuryMultiplier({ ...HALIBURTON, age: VALUATION_CONFIG.injury.unknownAge }),
    );
  });
});
...
describe("load management is not an injury", () => {
  ...
  it("costs a rested player exactly nothing", () => {
    ...
    expect(injuryMultiplier({ status: "DTD", bodyPart: "Not Injury Related" })).toBe(1);
    expect(injuryAssessment(MALUACH_REST).loadManagement).toBe(true);
  });

  it("still treats Undisclosed as an injury, because it is one", () => {
    const undisclosed = injuryMultiplier({ status: "DTD", bodyPart: "Undisclosed" });
  ...
  it("keeps illness a rounding error rather than a dynasty event", () => {
    expect(injuryMultiplier({ status: "DTD", bodyPart: "Illness" })).toBeGreaterThan(
      0.99,
    );
  });
});
