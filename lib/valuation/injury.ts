...
/** Everything the injury term reads about a player. */
export interface InjuryInput {
  /** Sleeper `injury_status`. Null for the ~94% of players carrying no flag. */
  status?: string | null;
  /** Sleeper `injury_body_part`. Present on every live flag; drives the class. */
  bodyPart?: string | null;
  /** Sleeper `injury_notes`. Present on 78 of 120 live flags; drives severity. */
  notes?: string | null;
  /** Current age. For a CURRENT injury this is also the age-at-injury (see above). */
  age?: number | null;
}

/** The full working of one player's injury term, for the UI and for tests. */
export interface InjuryAssessment {
  /** True when the player carries no flag at all. Everything else is then neutral. */
  healthy: boolean;
  injuryClass: InjuryClass;
  /** Share of dynasty value this costs, 0..1. */
  penalty: number;
  multiplier: number;
  classPenalty: number;
  noteScale: number;
  ...
  ageScale: number;
  /**
   * True for "Rest" and "Not Injury Related": flagged, but not hurt. Callers that
   * render an injury badge should skip these rather than label a rested rookie injured.
   */
  loadManagement: boolean;
}

/** Which class a Sleeper body-part string belongs to. */
export function injuryClassOf(
 ...
 * own.
 */
export function injuryAgeScale(
...
export function injuryAssessment(
  ...
  const classPenalty = i.classPenalty[injuryClass] ?? i.classPenalty[i.unmappedClass];
 ...
 */
export function maxInjuryMultiplier(
  cfg: ValuationConfig = VALUATION_CONFIG,
): number {
  const i = cfg.injury;
  ...
  const statusMin = Math.min(i.statusDefaultScale, ...Object.values(i.statusScale));
  const scales = [
...
export const INJURY_CLASS_LABELS: Record<InjuryClass, string> = {
  achilles: "Achilles",
