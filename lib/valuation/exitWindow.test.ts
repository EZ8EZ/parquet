...
import type { Player, Transaction } from "../providers/types";
import {
  VALUATION_CONFIG,
  ageMultiplier,
  positionMultipliers,
...
import { SUFFICIENCY, ageBlindConfig, deriveExitWindow } from "./exitWindow";

const SCORING = { pts: 0.5, reb: 1, ast: 1, stl: 2, blk: 2, to: -1, tpm: 0.5 };

function player(id: string, age: number, searchRank: number): Player {
  return {
    playerId: id,
    fullName: `Player ${id}`,
    firstName: "Player",
...
function trade(
  ...
  draftPicks: Transaction["draftPicks"] = [],
): Transaction {
  return {
    transactionId: id,
    type: "trade",
...
function history(players: Player[], transactions: Transaction[]): LeagueHistory {
  return {
    currentLeague: {
      season: "2026",
    ...
    const picky = deriveExitWindow(
      history(players, [
        trade("t2", "2026", { a: 1, b: 2 }, { a: 2, b: 1 }, [
          { round: 1, season: "2027", rosterId: 1, ownerId: 2, previousOwnerId: 1 },
        ]),
      ]),
    );
...
describe("deriveExitWindow: the refusal", () => {
  it("refuses on a sample where one deal carries a bucket", () => {
    ...
    const h = history(players, [
      trade("t1", "2026", { in1: 1, in2: 1, in3: 1, out: 2 }, { in1: 2, in2: 2, in3: 2, out: 1 }),
    ]);
    const w = deriveExitWindow(h);
  ...
  it("refuses a bucket that has the count but is carried by one acquisition", () => {
    ...
    const adds: Record<string, number> = { out: 2 };
    const drops: Record<string, number> = { out: 1 };
    for (const p of ins) {
      adds[p.playerId] = 1;
      drops[p.playerId] = 2;
    }
    const w = deriveExitWindow(history([...ins, out], [trade("t1", "2026", adds, drops)]));
    const bucket = w.buckets.find((b) => b.label === "30 to 31")!;
  ...
  it("is not hardcoded to refuse: a thick, evenly spread bucket clears the bar", () => {
    ...
    expect(bucket.concentration).toBeLessThanOrEqual(SUFFICIENCY.maxConcentration);
...
describe("D28: recalibrating the curve did not move the ceiling", () => {
  ...
  const HAND_SET: Array<[number, number]> = [
    [19, 1.16],
    [21, 1.14],
    [23, 1.08],
    [25, 1.02],
  ...
  it("still prices the most extreme hypothetical player at exactly maxValue", () => {
    const posMults = positionMultipliers(SCORING);
    const bestPos = Object.entries(posMults).reduce((b, e) => (e[1] > b[1] ? e : b))[0];
    const peakAge = VALUATION_CONFIG.ageAnchors.reduce((b, a) => (a[1] > b[1] ? a : b))[0];
    const synthetic = {
  ...
  it("moved the shape and only the shape: older assets up, the prime slightly down", () => {
    const before = { ...VALUATION_CONFIG, ageAnchors: HAND_SET };
    const at = (age: number, cfg = VALUATION_CONFIG) =>
