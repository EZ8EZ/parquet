...
export interface DerivedAgeRow {
  age: number;
  /** Discounted remaining relative production, scaled so the curve's peak is 1.16. */
  multiplier: number;
  /** Player-seasons observed starting at this age. The sample behind the row. */
  cohort: number;
  /** Share of that cohort still clearing the bar one season later. */
  stillPlaying: number;
}
 ...
 */
export const DERIVED_AGE_CURVE: readonly DerivedAgeRow[] = [
...
export function firstCliffAge(
  rows: readonly DerivedAgeRow[] = DERIVED_AGE_CURVE,
  referenceAge = 27,
  factor = 1.5,
): number | null {
  const drops: Array<{ age: number; drop: number }> = [];
  for (let i = 1; i < rows.length; i++) {
    const prev = rows[i - 1].multiplier;
    drops.push({ age: rows[i].age, drop: prev > 0 ? 1 - rows[i].multiplier / prev : 0 });
  }
  const moving = drops.map((d) => d.drop).filter((d) => d > 0).sort((a, b) => a - b);
  if (!moving.length) return null;
  const median = moving[Math.floor(moving.length / 2)];
  const hit = drops.find((d) => d.age > referenceAge && d.drop >= factor * median);
  return hit?.age ?? null;
}
 ...
 */
export function pastFirstCliff(
