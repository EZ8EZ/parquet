  ...
  const valued = (roster?.players ?? [])
    .map((pid) => {
      ...
      return {
        ...
        // Same two-rank pair /values carries, so the shared `ValueAssetRow` tells the
        // same story on both surfaces rather than disclosing the fallback on one page
        // and staying silent about the same player on the other.
        pricedRank: v.rank,
        productionBacked: v.productionBacked,
        productionRefusal: v.productionBacked
          ? null
          : refusalSentence(
              productionRowRefusal(p.fullName, rosteredWeeksBelowFloor(pid)),
            ),
        breakdown: {
          base: v.base,
          age: v.ageMultiplier,
