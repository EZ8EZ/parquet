...
describe("legacy localStorage parse (migration read only)", () => {
  it("reads the shape the pre-cookie board wrote", () => {
    ...
    expect(parseCustomOrder(JSON.stringify(["c", "a", "b"]))).toEqual(["c", "a", "b"]);
...
describe("cookie codec", () => {
  ...
  it("drops ids that could not be real player ids", () => {
    ...
    expect(parseCustomOrderCookie("v1.2577~bad;id~1658")).toEqual(["2577", "1658"]);
    expect(encodeCustomOrderCookie(["2577", "a,b", "1658"])).toBe("v1.2577~1658");
  });
  ...
  it("drops duplicates, which would otherwise give one player two ranks", () => {
    ...
    expect(parseCustomOrderCookie("v1.2577~1658~2577")).toEqual(["2577", "1658"]);
  });

  it("caps the order on both encode and parse so the cookie cannot grow unbounded", () => {
    const many = Array.from({ length: MAX_RANKED_IN_COOKIE + 50 }, (_, i) => `p${i}`);
    expect(encodeCustomOrderCookie(many).split("~")).toHaveLength(MAX_RANKED_IN_COOKIE);
    const oversized = `v1.${many.join("~")}`;
    expect(parseCustomOrderCookie(oversized)).toHaveLength(MAX_RANKED_IN_COOKIE);
  });
...
describe("customOrderFromCookieHeader (the client's read path)", () => {
  ...
  it("reads a header containing only this cookie, with and without padding", () => {
    expect(customOrderFromCookieHeader(`${CUSTOM_RANK_COOKIE}=${value}`)).toEqual([
      "2577",
      "1658",
      "p3",
    ]);
