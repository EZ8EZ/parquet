...
export function isSafePlayerId(id: string): boolean {
 ...
 */
export function encodeCustomOrderCookie(orderedPlayerIds: string[]): string {
  const ids = orderedPlayerIds.filter(isSafePlayerId).slice(0, MAX_RANKED_IN_COOKIE);
 ...
 */
export function parseCustomOrderCookie(raw: string | null | undefined): string[] {
  if (!raw) return [];
 ...
 */
export function customOrderFromCookieHeader(
