 ...
 */
import { CUSTOM_RANK_COOKIE, parseCustomOrderCookie } from "./customOrder";
 ...
 */
export async function readCustomOrder(): Promise<string[]> {
  try {
    const { cookies } = await import("next/headers");
    return parseCustomOrderCookie((await cookies()).get(CUSTOM_RANK_COOKIE)?.value);
  } catch {
    return [];
  }
}
