...
describe("startableAfter", () => {
  it("drops what you send and adds what you get", () => {
    expect(
      startableAfter(["a", "b", "c"], [asAsset("b")], [asAsset("z")]).sort(),
    ).toEqual(["a", "c", "z"]);
  });
  /** A pick cannot fill a slot tonight, so it is not startable depth (see fragility.ts). */
  it("ignores incoming picks entirely", () => {
    /** @type {{ kind: "pick", id: string, label: string, value: number, age: null, position: null, pick: { round: number, season: string, originalRosterId: number } }} */
    const incomingPick = {
      kind: "pick",
      id: "2028-1-9",
      label: "2028 R1",
    ...
    expect(startableAfter(["a"], [], [incomingPick])).toEqual(["a"]);
  });
});
/** @returns {{ kind: "player", id: string, label: string, value: number, age: number, position: string }} */
function asAsset(id) {
  return {
    kind: "player",
    id,
