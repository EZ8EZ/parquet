...
import { cachedLeagueTimelines, getTimelineProfile } from "../metrics/duration.js";
...
import { parseTradeParams } from "../trade/url.js";
// --------------------------------------------------------------------- builders
/** @returns {import('../trade/index.js').TradeAsset} */
function player(id, value, age, position = "SF") {
  return { kind: "player", id, label: `P-${id}`, value, age, position };
}
/** @returns {import('../trade/index.js').TradeAsset} */
function pick(id, value, season = "2028", round = 1) {
  return {
    kind: "pick",
    id,
 ...
 * @param {Partial<import('./index.js').Appetite>} [over]
 * @returns {import('./index.js').Appetite}
 */
function appetite(over = {}) {
  return {
    rosterId: 1,
    name: "Team 1",
    stance: "retool",
...
describe("perceive: what a side thinks an asset is worth", () => {
  ...
  it("stops paying a win-now premium for players past their prime", () => {
    ...
    expect(perceive(player("a", 1000, 33), contender).value).toBeGreaterThan(
      1000,
    );
  ...
  it("prices the same young player in opposite directions", () => {
    ...
    expect(
      perceive(kid, appetite({ wantsFuture: true })).value,
    ).toBeGreaterThan(2000);
    expect(perceive(kid, appetite({ wantsNow: true })).value).toBeLessThan(
      2000,
    );
  });
  it("prices picks up for a hoarder, down for a contender, and up for a team ahead of schedule", () => {
    ...
    expect(perceive(p, appetite({ picksAreAmmo: true })).value).toBeGreaterThan(
      1500,
    );
  });
  it("applies positional need as a premium and positional surplus as a discount", () => {
    ...
    expect(
      perceive(c, appetite({ weakPositions: ["C"] })).value,
    ).toBeGreaterThan(1000);
    expect(
      perceive(c, appetite({ strongPositions: ["C"] })).value,
    ).toBeLessThan(1000);
  ...
  it("never lets stacked tells move an asset more than the clamp", () => {
    ...
    expect(
      perceive(player("s", 5000, 32, "SF"), stacked).value,
    ).toBeLessThanOrEqual(Math.round(5000 * (1 + FIT_CLAMP)));
    const negative = appetite({ wantsFuture: true, strongPositions: ["SF"] });
    expect(
      perceive(player("s", 5000, 34, "SF"), negative).value,
    ).toBeGreaterThanOrEqual(Math.round(5000 * (1 - FIT_CLAMP)));
  });
  ...
  it("writes the viewer's reasons in the second person and the partner's in the third", () => {
    ...
    const yours = perceive(
      asset,
      appetite({ viewer: true, weakPositions: ["C"] }),
    );
    const theirs = perceive(
      asset,
      appetite({ viewer: false, weakPositions: ["C"] }),
    );
...
describe("stance and appetite", () => {
  it("reads an old, top-half, star-heavy roster as contending", () => {
    // Two cornerstone-tier assets, using /plan's own threshold for the word. The
    // standing override, which is the one thing that can put the stance somewhere the
    // posture does not point.
    ...
    const ap = appetiteFor(a, 2, 14, { posture: "ascending" });
  ...
  it("reads a young top-half roster as ascending, not as a seller of youth or picks", () => {
    ...
    // roster good.
    const ap = appetiteFor(analysis(), 3, 14, { posture: "rebuilding" });
  ...
  it("reads a young bottom-half roster as rebuilding, picks included", () => {
    const ap = appetiteFor(analysis(), 12, 14, { posture: "rebuilding" });
  ...
  /**
   * The anti-drift guard. There is now ONE `stanceOf` (lib/metrics/axes.js) and both
   * engines call it, so this can no longer catch two implementations drifting - what it
   * still catches is the two engines feeding that function different arguments, which
   * is the other way this page and /plan could print different words for one roster.
   */
  it("agrees with /plan's diagnosis on every roster in the league", async () => {
    ...
    ranking.forEach((a, i) => {
      const ap = appetiteFor(a, i + 1, ranking.length, {
        posture: postures.get(a.rosterId) ?? null,
      });
      expect(ap.stance).toBe(diagnose(h, a.rosterId, principals).direction);
    });
  });
...
function run(mine, theirs, you, partner, max = 3, opts = {}) {
...
const top = (list) => Math.max(...list.map((p) => p.asset.value));
describe("searchPackages invariants", () => {
  ...
  it("lets a REBUILDING side sell its star for a bundle, with the value band as the guard", () => {
    ...
    const starRoster = [
      player("star", 5000, 31, "SF"),
      player("dep", 600, 27, "PG"),
    ];
    const buyerAssets = [
      player("b1", 2600, 22, "C"),
  ...
  it("finds nothing when both sides perceive value identically", () => {
    expect(
      run(
        mine,
        theirs,
        appetite({ rosterId: 1, viewer: true }),
        appetite({ rosterId: 2 }),
        4,
      ),
    ).toHaveLength(0);
  ...
  it("surfaces one idea per headline target, not four spellings of one offer", () => {
    const heads = run(mine, theirs, contender, rebuilder, 4).map(
      (p) => p.get[0].asset.id,
    );
  ...
  it("keeps the reported totals consistent with the assets in the package", () => {
    for (const pkg of run(mine, theirs, contender, rebuilder, 4)) {
      const give = pkg.give.reduce((s, p) => s + p.asset.value, 0);
      const get = pkg.get.reduce((s, p) => s + p.asset.value, 0);
  ...
  it("routes an aging asset toward the partner whose record says they pay for age", () => {
    ...
    const chaser = appetite({
      rosterId: 2,
      weakPositions: ["SF"],
      paysForAge: true,
    });
    const pool = [
      player("old", 1800, 32, "PF"),
      player("m1", 1800, 24, "PF"),
      pick("mp1", 1700),
    ];
...
describe("findTrades against the fixture league", () => {
  ...
  it("returns nothing for a trade with yourself", async () => {
    const principals = await getPrincipals(h);
    expect(
      findTrades(h, principals, { rosterId: me, partnerRosterId: me }),
    ).toBeNull();
  });
  /**
  ...
  it("attaches conviction notes only to players actually in the package", async () => {
    ...
    for (const other of h.rosters) {
      ...
      for (const pkg of r?.packages ?? []) {
        const ids = new Map([
          ...pkg.give.map((a) => /** @type {[string, string]} */ ([a.id, "give"])),
          ...pkg.get.map((a) => /** @type {[string, string]} */ ([a.id, "get"])),
        ]);
  ...
  it("argues both sides of every package, and never quotes a discount as an argument", async () => {
    ...
    for (const other of h.rosters) {
      ...
      for (const pkg of r?.packages ?? []) {
        ...
        for (const line of pkg.yourCase)
          expect(line).not.toContain("their thinnest");
...
/*
 * `move=` - THE FORCED GIVE POOL.
 *
 * The give pool is partner-driven (`price` sorts by gap), so the viewer's own diagnosed
 * problem asset can be structurally unreachable by any suggestion. These pin the three
 * things the force has to be: honoured on every package it returns, honest about owning
 * nothing, and free of side effects on an unforced search.
 */
describe("searchPackages with a forced give asset", () => {
  const you = appetite({
    rosterId: 1,
    viewer: true,
    stance: "contend",
  ...
  // Twelve assets, so the default pool of ten genuinely excludes the tail and the
  // "displaced into the pool" branch is the one under test rather than a coincidence.
  const mine = [
    player("m1", 2400, 22, "PG"),
    player("m2", 2100, 23, "SG"),
    player("m3", 1400, 24, "SF"),
    player("m4", 1300, 24, "PF"),
    player("m5", 1200, 23, "C"),
    player("m6", 1100, 22, "PG"),
    player("m7", 1000, 24, "SG"),
    player("m8", 950, 23, "SF"),
    player("m9", 900, 22, "PF"),
    player("m10", 850, 24, "C"),
    player("old", 800, 34, "PF"),
    pick("mp1", 1600),
  ];
  const theirs = [
    player("t1", 5200, 29, "C"),
  ...
  it("puts the forced asset in every package it returns", () => {
    const out = run(mine, theirs, you, them, 4, { forceGiveId: "old" });
    expect(out.length).toBeGreaterThan(0);
    for (const pkg of out)
      expect(pkg.give.some((p) => p.asset.id === "old")).toBe(true);
  });
  it("reaches an asset the unforced pool never offers", () => {
    // A 34-year-old is the last thing a rebuilder wants, so the gap sort buries him.
    const unforced = run(mine, theirs, you, them, 4);
    const buried = unforced.every(
      (pkg) => !pkg.give.some((p) => p.asset.id === "old"),
    );
    const forced = run(mine, theirs, you, them, 4, { forceGiveId: "old" });
    // Either he was already unreachable and forcing surfaced him, or the fixture
    // happens to offer him anyway - in both cases forcing must not LOSE him.
    if (buried) expect(forced.length).toBeGreaterThan(0);
    for (const pkg of forced)
      expect(pkg.give.some((p) => p.asset.id === "old")).toBe(true);
  });
  it("returns nothing at all for an asset the roster does not hold", () => {
    expect(run(mine, theirs, you, them, 4, { forceGiveId: "nobody" })).toEqual(
      [],
    );
  });
  it("leaves an unforced search byte-identical", () => {
    expect(run(mine, theirs, you, them, 3, {})).toEqual(
      run(mine, theirs, you, them, 3),
    );
    expect(run(mine, theirs, you, them, 3, { forceGiveId: null })).toEqual(
      run(mine, theirs, you, them, 3),
    );
  });
  it("never widens the give pool past its own cap", () => {
    // Forcing displaces the pool's last entry rather than adding an eleventh, so a
    // forced search cannot cost more combinations than an ordinary one.
    for (const pkg of run(mine, theirs, you, them, 4, { forceGiveId: "mp1" }))
      expect(pkg.give.length).toBeLessThanOrEqual(3);
  });
  /*
   * TWO FORCED IDS AT ONCE (D113) - /plan's timeline break can now name a correlated
   * pair rather than a single asset, and `move=` carries both.
   */
  it("accepts an array of ids and puts every one of them in every package it returns", () => {
    const out = run(mine, theirs, you, them, 4, {
      forceGiveId: ["old", "mp1"],
    });
    expect(out.length).toBeGreaterThan(0);
    for (const pkg of out) {
      expect(pkg.give.some((p) => p.asset.id === "old")).toBe(true);
      expect(pkg.give.some((p) => p.asset.id === "mp1")).toBe(true);
    }
  });
  it("returns nothing if even one of two forced ids is not on the roster", () => {
    expect(
      run(mine, theirs, you, them, 4, { forceGiveId: ["old", "nobody"] }),
    ).toEqual([]);
  });
  it("still never widens the pool past its cap with two ids forced", () => {
    // Both "old" and "mp1" sit outside the unforced top-ten pool on this fixture, so
    // this exercises two displacements in the same search, not one.
    for (const pkg of run(mine, theirs, you, them, 4, {
      forceGiveId: ["old", "mp1"],
    }))
      expect(pkg.give.length).toBeLessThanOrEqual(3);
  });
  it("gives the identical result whichever order the two forced ids are given in", () => {
    expect(
      run(mine, theirs, you, them, 4, { forceGiveId: ["old", "mp1"] }),
    ).toEqual(run(mine, theirs, you, them, 4, { forceGiveId: ["mp1", "old"] }));
  });
});
describe("findTrades with move=", () => {
  const h = buildFixtureHistory();
  const me = h.me.rosterId;
  const other = h.rosters.find((r) => r.rosterId !== me).rosterId;
  it("names the asset it was asked to move, and honours it", async () => {
    const principals = await getPrincipals(h);
    const mineIds = assetsOf(analyzeRoster(h, me)).map((a) => a.id);
    let checked = 0;
    for (const id of mineIds.slice(0, 6)) {
      const r = findTrades(h, principals, {
        rosterId: me,
        partnerRosterId: other,
        move: id,
      });
      expect(r.move.requested).toBe(String(id));
      expect(r.move.asset.id).toBe(id);
      for (const pkg of r.packages) {
        expect(pkg.give.some((a) => String(a.id) === String(id))).toBe(true);
        checked++;
      }
    }
    // The fixture must actually exercise the constrained path somewhere, or this
    // whole block would be asserting over empty lists.
    expect(checked).toBeGreaterThan(0);
  });
  it("refuses in words rather than returning a bare empty list", async () => {
    const principals = await getPrincipals(h);
    const r = findTrades(h, principals, {
      rosterId: me,
      partnerRosterId: other,
    ...
    expect(r.move.asset).toBeNull();
    expect(r.caveats.some((c) => /not on your roster/i.test(c))).toBe(true);
  });
  it("says which question it answered when a forced search finds nothing", async () => {
    const principals = await getPrincipals(h);
    // The lowest-gap asset on the roster is the likeliest to clear nothing anywhere.
    const mine = assetsOf(analyzeRoster(h, me));
    const cheapest = [...mine].sort((a, b) => a.value - b.value)[0];
    for (const partnerRosterId of h.rosters
      .map((r) => r.rosterId)
      .filter((id) => id !== me)) {
      const r = findTrades(h, principals, {
        rosterId: me,
      ...
      if (r.packages.length === 0) {
        // Names BOTH the partner and the asset: "nothing works with them" and
        // "nothing works with them that includes him" are different answers.
        expect(
          r.caveats.some(
            (c) => c.includes(cheapest.label) && c.includes(r.partner.name),
          ),
        ).toBe(true);
        return;
      }
  ...
  it("leaves the unforced result untouched", async () => {
    const principals = await getPrincipals(h);
    const base = { rosterId: me, partnerRosterId: other, max: 3 };
    const a = findTrades(h, principals, base);
    const b = findTrades(h, principals, { ...base, move: null });
    expect(b.packages.map((p) => p.headline)).toEqual(
      a.packages.map((p) => p.headline),
    );
    expect(a.move).toBeNull();
  });
  /*
   * A COMMA-JOINED PAIR (D113) - `/plan`'s timeline break can name two correlated
   * assets rather than one, and this is the extension that lets its link force both.
   */
  it("forces both assets of a comma-joined pair into every package, across every partner", async () => {
    const principals = await getPrincipals(h);
    // The fixture's own real correlated pair, rather than an arbitrary two assets -
    // forcing the roster's two highest-value pieces at once clears no partner's bar,
    // which is a real finding but not what this test needs to exercise the give pool.
    const brk = getTimelineProfile(h, me).timelineBreak;
    expect(brk?.assets.length).toBe(2);
    const mine = assetsOf(analyzeRoster(h, me));
    const [a1, a2] = brk.assets.map(
    ...
    for (const partnerRosterId of h.rosters
      ...
      expect(r.move.requested).toBe(`${a1.id},${a2.id}`);
      expect(new Set(r.move.assets.map((a) => String(a.id)))).toEqual(
        new Set([String(a1.id), String(a2.id)]),
      );
      // `move.asset` (singular) is unchanged for every existing single-id caller.
      expect(String(r.move.asset.id)).toBe(String(a1.id));
      for (const pkg of r.packages) {
        expect(pkg.give.some((a) => String(a.id) === String(a1.id))).toBe(true);
        expect(pkg.give.some((a) => String(a.id) === String(a2.id))).toBe(true);
        checked++;
      }
    }
    expect(checked).toBeGreaterThan(0);
  });
  it("reports move.assets as null, never a partial list, when only one of two ids is owned", async () => {
    const principals = await getPrincipals(h);
    const mine = assetsOf(analyzeRoster(h, me));
    const owned = mine[0];
    const r = findTrades(h, principals, {
      rosterId: me,
      partnerRosterId: other,
      move: `${owned.id},not-an-asset`,
    });
    expect(r.packages).toEqual([]);
    expect(r.move.assets).toBeNull();
    // The one id that DID resolve is still nameable via the singular field, so a
    // caller that only reads `move.asset` (every pre-D113 caller) is unaffected.
    expect(String(r.move.asset.id)).toBe(String(owned.id));
    expect(
      r.caveats.some((c) => /not on your roster/i.test(c)),
    ).toBe(true);
  });
});
describe("partnerBoard with move=", () => {
  const h = buildFixtureHistory();
  const me = h.me.rosterId;
  it("constrains every partner's best idea to packages holding that asset", async () => {
    const principals = await getPrincipals(h);
    const mine = assetsOf(analyzeRoster(h, me));
    const target = [...mine].sort((a, b) => b.value - a.value)[0];
    const rows = partnerBoard(h, principals, me, { move: target.id });
    expect(rows.length).toBe(h.rosters.length - 1);
    for (const r of rows) {
      if (r.bestIdea) expect(r.bestIdea).toContain(target.label);
      else expect(r.mutual).toBe(0);
    }
  });
  it("finds nothing anywhere for an asset the viewer does not own", async () => {
    const principals = await getPrincipals(h);
    const rows = partnerBoard(h, principals, me, { move: "not-an-asset" });
    expect(rows.length).toBe(h.rosters.length - 1);
    for (const r of rows) {
      expect(r.mutual).toBe(0);
      expect(r.bestIdea).toBeNull();
    }
  });
  it("constrains every partner's best idea to packages holding BOTH assets of a comma-joined pair", async () => {
    const principals = await getPrincipals(h);
    const mine = assetsOf(analyzeRoster(h, me));
    const [a1, a2] = [...mine].sort((a, b) => b.value - a.value).slice(0, 2);
    const rows = partnerBoard(h, principals, me, {
      move: `${a1.id},${a2.id}`,
    });
    expect(rows.length).toBe(h.rosters.length - 1);
    for (const r of rows) {
      if (r.bestIdea) {
        expect(r.bestIdea).toContain(a1.label);
        expect(r.bestIdea).toContain(a2.label);
      } else expect(r.mutual).toBe(0);
    }
  });
  it("finds nothing anywhere when only one of two requested ids is owned", async () => {
    const principals = await getPrincipals(h);
    const mine = assetsOf(analyzeRoster(h, me));
    const rows = partnerBoard(h, principals, me, {
      move: `${mine[0].id},not-an-asset`,
    });
    expect(rows.length).toBe(h.rosters.length - 1);
    for (const r of rows) {
      expect(r.mutual).toBe(0);
...
describe("viewerAsset", () => {
  const h = buildFixtureHistory();
  const me = h.me.rosterId;
  it("resolves the viewer's own players and picks, and nothing else", () => {
    for (const a of assetsOf(analyzeRoster(h, me))) {
      expect(viewerAsset(h, me, a.id).label).toBe(a.label);
    }
    expect(viewerAsset(h, me, "nope")).toBeNull();
    expect(viewerAsset(h, me, null)).toBeNull();
    // Somebody else's player is not "yours" just because it exists.
    const other = h.rosters.find((r) => r.rosterId !== me).rosterId;
    const theirs = assetsOf(analyzeRoster(h, other)).filter(
      (a) => a.kind === "player",
    );
    for (const a of theirs.slice(0, 5)) expect(viewerAsset(h, me, a.id)).toBeNull();
  });
});
/*
 * THE ROOM BANDS. `mutual` is a fit-adjusted delta on a clamped multiplier, so it has no
 * absolute scale and its third digit is noise - hence three bands off the board's own
 * terciles rather than a printed figure. See `roomBands` for the full argument.
 */
describe("roomBands / roomBand", () => {
  it("refuses to band fewer than three live values", () => {
    expect(roomBands([])).toBeNull();
    expect(roomBands([0, 0, 0])).toBeNull();
    expect(roomBands([500, 0, 900])).toBeNull();
    expect(roomBand(500, null)).toBeNull();
  });
  it("splits a board into three bands, low to high", () => {
    const values = [100, 200, 300, 400, 500, 600, 700, 800, 900];
    const bands = roomBands(values);
    expect(bands.n).toBe(9);
    expect(roomBand(100, bands)).toBe("narrow");
    expect(roomBand(500, bands)).toBe("real");
    expect(roomBand(900, bands)).toBe("wide");
  });
  it("is relative to the board it was given, never absolute", () => {
    const small = roomBands([10, 20, 30]);
    const large = roomBands([10000, 20000, 30000]);
    // The same figure is "wide" on one board and "narrow" on the other, which is the
    // whole point: there is no absolute scale for this quantity to be measured on.
    expect(roomBand(30, small)).toBe("wide");
    expect(roomBand(30, large)).toBe("narrow");
  });
  it("never bands a row with no room at all", () => {
    const bands = roomBands([100, 200, 300]);
    expect(roomBand(0, bands)).toBeNull();
    expect(roomBand(-5, bands)).toBeNull();
  });
  it("bands the real fixture board without gaps or overlaps", async () => {
    const h = buildFixtureHistory();
    const principals = await getPrincipals(h);
    const rows = partnerBoard(h, principals, h.me.rosterId);
    const bands = roomBands(rows.map((r) => r.mutual));
    for (const r of rows) {
      const band = roomBand(r.mutual, bands);
      if (r.mutual > 0 && bands) expect(band).not.toBeNull();
      if (!(r.mutual > 0)) expect(band).toBeNull();
    }
  });
});
/*
 * THE LINK `tradeHref` WAS BUILT FOR AND NEVER GIVEN. Every suggested package pointed
 * at a bare `/trade`; the codec that solved exactly this was one import away with a test
 * suite and zero production callers.
 */
describe("packageBuilderHref", () => {
  it("round-trips a package's assets through the /trade codec", () => {
    const give = [player("g1", 900, 24), pick("2028-1-9", 1600)];
    const get = [player("t1", 2400, 22)];
    const href = packageBuilderHref(give, get);
    expect(href.startsWith("/trade?")).toBe(true);
    const parsed = parseTradeParams(
  ...
  it("gives every real package on the fixture league a resolvable link", async () => {
    const h = buildFixtureHistory();
    const principals = await getPrincipals(h);
    const me = h.me.rosterId;
    let seen = 0;
    for (const r of h.rosters.filter((x) => x.rosterId !== me)) {
      const result = findTrades(h, principals, {
        rosterId: me,
        partnerRosterId: r.rosterId,
      });
      for (const pkg of result.packages) {
        seen++;
        const parsed = parseTradeParams(
          new URLSearchParams(pkg.builderHref.slice(pkg.builderHref.indexOf("?"))),
        );
        // Every id in the link is an id the package actually contains, both sides.
        const giveIds = pkg.give.map((a) => String(a.id));
        const getIds = pkg.get.map((a) => String(a.id));
        for (const id of [...parsed.give, ...parsed.givePicks])
          expect(giveIds).toContain(id);
        for (const id of [...parsed.get, ...parsed.getPicks])
          expect(getIds).toContain(id);
        // And nothing was dropped on the way.
        expect(parsed.give.length + parsed.givePicks.length).toBe(giveIds.length);
        expect(parsed.get.length + parsed.getPicks.length).toBe(getIds.length);
      }
    }
    expect(seen).toBeGreaterThan(0);
  });
});
describe("the two priced lists findTrades used to discard", () => {
  const h = buildFixtureHistory();
  const me = h.me.rosterId;
  it("returns both sides' assets, priced and gap-sorted", async () => {
    const principals = await getPrincipals(h);
    const other = h.rosters.find((r) => r.rosterId !== me).rosterId;
    const r = findTrades(h, principals, {
      rosterId: me,
    ...
    expect(r.mine.length).toBe(assetsOf(analyzeRoster(h, me)).length);
    expect(r.theirs.length).toBe(assetsOf(analyzeRoster(h, other)).length);
    for (const list of [r.mine, r.theirs]) {
      for (let i = 1; i < list.length; i++)
        expect(list[i - 1].gap).toBeGreaterThanOrEqual(list[i].gap);
      // The gap IS the subtraction, not a score derived from one.
      for (const p of list)
        expect(p.gap).toBe(p.taker.value - p.holder.value);
    }
  });
  it("only ever lists assets the two sides actually own", async () => {
    const principals = await getPrincipals(h);
    const other = h.rosters.find((r) => r.rosterId !== me).rosterId;
    const r = findTrades(h, principals, {
      rosterId: me,
    ...
    const mineIds = new Set(assetsOf(analyzeRoster(h, me)).map((a) => a.id));
    const theirIds = new Set(assetsOf(analyzeRoster(h, other)).map((a) => a.id));
    for (const p of r.mine) expect(mineIds.has(p.asset.id)).toBe(true);
    for (const p of r.theirs) expect(theirIds.has(p.asset.id)).toBe(true);
  });
});
describe("the stance sentence, said once", () => {
  const h = buildFixtureHistory();
  const me = h.me.rosterId;
  it("is on the result, not appended to every package's case", async () => {
    const principals = await getPrincipals(h);
    for (const r of h.rosters.filter((x) => x.rosterId !== me)) {
      const result = findTrades(h, principals, {
        rosterId: me,
      ...
      expect(result.stanceNote).toBe(stanceNoteFor(result.partner));
      expect(result.stanceNote.length).toBeGreaterThan(0);
      // The four canned lines are gone from the per-package lists entirely.
      for (const pkg of result.packages) {
        expect(pkg.theirCase).not.toContain(result.stanceNote);
        for (const line of pkg.theirCase)
          expect(line).not.toMatch(
            /They read as (win-now|rebuilding)|ahead of schedule, so they want|stuck mid-pack/,
          );
      }
    }
  });
  it("covers all four stances", () => {
    const notes = new Set(
      ["contend", "rebuild", "ascend", "retool"].map((stance) =>
        stanceNoteFor(
          appetite({
            stance,
