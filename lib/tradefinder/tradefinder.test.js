...
describe("searchPackages with a forced give asset", () => {
  ...
  /*
   * TWO FORCED IDS AT ONCE (D114) - /plan's timeline break can now name a correlated
...
describe("findTrades with move=", () => {
  ...
  /*
   * A COMMA-JOINED PAIR (D114) - `/plan`'s timeline break can name two correlated
  ...
  it("reports move.assets as null, never a partial list, when only one of two ids is owned", async () => {
    ...
    // caller that only reads `move.asset` (every pre-D114 caller) is unaffected.
