...
import { spofOfPlayers, startableRosterIds, type SpofRead } from "../metrics/fragility";
import {
  fragilityNoteFor,
...
import { findTrades, type FinderAsset } from "./index";

const h = buildFixtureHistory();
const ME = h.me.rosterId!;

function read(name: string, damageShare: number, playerId = name): SpofRead {
  return {
    playerId,
    name,
...
const byName = new Map(
  leagueValueRanking(h).flatMap((r) => r.valued.map((v) => [v.name, v] as const)),
);

function asset(name: string): FinderAsset {
...
describe("fragilityNoteFor", () => {
  it("reads a falling share as relief, and names both men when the load moves", () => {
    const note = fragilityNoteFor(read("Luka Doncic", 0.4), read("Scottie Barnes", 0.2));
  ...
  it("reads a falling share on the SAME man as relief too", () => {
    const note = fragilityNoteFor(read("Luka Doncic", 0.4), read("Luka Doncic", 0.3));
    expect(note?.direction).toBe("relieves");
    expect(note?.text).toContain("leans less");
  });

  it("reads a rising share as a point of failure being created", () => {
    const note = fragilityNoteFor(read("Luka Doncic", 0.25), read("Nikola Jokic", 0.45));
  ...
  it("never grades the package in either direction", () => {
    const banned = /\b(good|bad|great|terrible|winner|loser|grade|avoid|do not make)\b/i;
    for (const note of [
...
describe("rosterAfter", () => {
  it("drops what you send and adds what you get", () => {
    expect(rosterAfter(["a", "b", "c"], [asAsset("b")], [asAsset("z")]).sort()).toEqual([
      "a",
      "c",
      "z",
    ]);
  ...
  it("ignores incoming picks entirely", () => {
    const incomingPick: FinderAsset = {
      kind: "pick",
      id: "2028-1-9",
      label: "2028 R1",
      value: 3000,
...
function asAsset(id: string): FinderAsset {
  return { kind: "player", id, label: id, value: 1000, age: 25, position: "SF" };
}
...
describe("packageFragilityNote over the fixture league", () => {
  ...
  it("returns null for a roster that does not exist rather than throwing", () => {
    expect(packageFragilityNote(h, 9999, [], [asset("Luka Doncic")])).toBeNull();
  });
});
