 ...
 *
 * ACCEPTS ONE ID OR SEVERAL, since D113 (`/plan`'s timeline break can now name a
 ...
 * (D113's `findTimelineBreak` can name two assets), a single id unchanged. Trims each
...
export function findTrades(h, principals, opts) {
  ...
  // `move=` can carry a comma-joined pair since D113 - `forcedAssets[0]` is unchanged
  ...
  return {
     ...
     * D113: the full resolved list when EVERY requested id is owned, `null` otherwise
 ...
 *   include the viewer's own asset(s) - one id, or a comma-joined pair since D113 - see
