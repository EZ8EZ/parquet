 ...
 *
 * ACCEPTS ONE ID OR SEVERAL, since D114 (`/plan`'s timeline break can now name a
 * correlated PAIR rather than a single asset). A single string keeps every existing
 * caller and test byte-identical; an array requires ALL of them present in every
 * package, for the same reason a single forced id is never optional - a search that
 * honoured only one of two requested ids would silently answer a narrower question
 * than the one asked. `MAX_GIVE_PIECES` (3) leaves room for both forced slots plus one
 * more, so a two-asset force still fits inside a normal-sized package.
 */
function normalizeForceIds(v) {
  if (v == null || v === "") return [];
 ...
 * @param {number} max
 * @param {{ forceGiveId?: string|string[]|null }} [opts]
 * @returns {TradePackageCandidate[]}
 */
export function searchPackages(mine, theirs, you, partner, max, opts = {}) {
  const forceIds = normalizeForceIds(opts.forceGiveId);
  let givePool = mine.slice(0, MAX_GIVE_POOL);
  const forced = [];
  for (const id of forceIds) {
    const already = givePool.find((p) => String(p.asset.id) === String(id));
    if (already) {
      forced.push(already);
      continue;
    }
    const found = mine.find((p) => String(p.asset.id) === String(id));
    if (!found) return [];
    // Displaces the pool's last entry rather than widening the pool, so the
    // combinatorial cost of a forced search is identical to an ordinary one, however
    // many ids are forced.
    givePool = [found, ...givePool.slice(0, MAX_GIVE_POOL - 1)];
    forced.push(found);
  }
  // Re-resolved by object identity AFTER every id has been placed, rather than
  // recorded as each one is found: an earlier id's index shifts every time a later
  // one gets prepended, and finding by reference is correct regardless of how many
  // times the pool was displaced.
  const forcedIndices = forced.map((f) => givePool.indexOf(f));
  const targetPool = theirs.slice(0, MAX_TARGETS);
  const giveSets = combos(givePool.length, MAX_GIVE_PIECES).filter(
    (si) => forcedIndices.every((fi) => si.includes(fi)),
  );
...
/**
 * `move=` split into individual ids - a comma-joined list for the correlated-pair case
 * (D114's `findTimelineBreak` can name two assets), a single id unchanged. Trims each
 * piece and drops empties, so a trailing comma or stray space in a hand-edited URL
 * cannot manufacture a phantom third id.
 * @param {string|null|undefined} move
 * @returns {string[]}
 */
export function parseMoveIds(move) {
  if (!move) return [];
  return String(move)
    .split(",")
    .map((s) => s.trim())
...
/**
 * `viewerAsset`, over every id `move=` named rather than just one - one `analyzeRoster`
 * call shared across all of them rather than one per id, since a page resolving a
 * two-asset break would otherwise pay for the same roster read twice. Each position of
 * the result lines up with `parseMoveIds(move)`, `null` where that id is not owned, so
 * a caller can tell WHICH of several requested ids failed to resolve rather than only
 * that one did.
 * @param {LeagueHistory} h
 * @param {number} rosterId
 * @param {string|null|undefined} move
 * @returns {(TradeAsset|null)[]}
 */
export function viewerAssets(h, rosterId, move) {
  const ids = parseMoveIds(move);
  if (!ids.length) return [];
  const a = analyzeRoster(h, rosterId);
  if (!a) return ids.map(() => null);
  const mine = assetsOf(a);
  return ids.map(
    (id) => mine.find((asset) => String(asset.id) === String(id)) ?? null,
  );
}
/**
...
export function findTrades(h, principals, opts) {
  ...
  // The forced asset(s), resolved against the viewer's own priced pool rather than
  // trusted from the URL: a `move=` param naming something this roster does not own has
  // to read as "not yours" rather than silently searching without the constraint.
  // `move=` can carry a comma-joined pair since D114 - `forcedAssets[0]` is unchanged
  // for the single-id case every existing caller still uses.
  const moveRequested = opts.move != null && opts.move !== "";
  const requestedIds = moveRequested ? parseMoveIds(opts.move) : [];
  const forcedAssets = requestedIds.map(
    (id) => mine.find((p) => String(p.asset.id) === String(id))?.asset ?? null,
  );
  const allOwned = moveRequested && forcedAssets.every((a) => a != null);
  // ANY unowned id searches for NOTHING rather than falling back to an unconstrained
  ...
  const raw =
    moveRequested && !allOwned
      ? []
      : searchPackages(mine, theirs, you, partner, opts.max ?? 3, {
          forceGiveId: allOwned ? forcedAssets.map((a) => a.id) : null,
        });
   ...
   * asset(s) reads as the finding it is.
   */
  if (!packages.length && moveRequested) {
    const names = forcedAssets
      .filter(Boolean)
      .map((a) => a.label)
      .join(" and ");
    const plural = forcedAssets.filter(Boolean).length > 1;
    caveats.push(
      allOwned
        ? `Nothing clears the bar with ${partner.name} that includes ${names}. That is a real answer: they would have to want ${plural ? "both" : names} enough to pay for ${plural ? "them" : "him"}, and on this reading they do not. Try another partner rather than a worse package.`
        : requestedIds.length > 1
          ? `One or more of those assets is not on your roster, so no package can include all of them. Nothing here was filtered - there was nothing to filter.`
  ...
  return {
     ...
     * tell "you do not own that" apart from "no package includes it". `asset` is
     * unchanged for every existing single-id caller - the first (and for a single id,
     * only) resolved asset, `null` if that id is not owned. `assets` is new since
     * D114: the full resolved list when EVERY requested id is owned, `null` otherwise
     * - never a partial list, because a page rendering "assets" off a partial result
     * would silently drop the one(s) that failed to resolve rather than saying so.
     */
    move: moveRequested
      ? {
          requested: String(opts.move),
          asset: forcedAssets[0] ?? null,
          assets: allOwned ? forcedAssets : null,
        }
      : null,
  };
}
 ...
 *   include the viewer's own asset(s) - one id, or a comma-joined pair since D114 - see
 *   `searchPackages`. A partner for whom no such package exists comes back with
 *   `mutual: 0`, which is the honest answer to the constrained question rather than the
 *   unconstrained one.
 */
export function partnerBoard(h, principals, rosterId, opts = {}) {
  ...
  const allOwned =
    moveRequested &&
    requestedIds.every((id) => myAssets.some((a) => String(a.id) === String(id)));
  const wins = windowsFor(h, rosterId);
  const rows = [];
    ...
    const best =
      moveRequested && !allOwned
        ...
        : (searchPackages(mine, theirs, you, partner, 1, {
            forceGiveId: moveRequested ? requestedIds : null,
          })[0] ?? null);
