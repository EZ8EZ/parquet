 ...
 * `coherenceOf` for the TCI either side of the deal, `findTimelineBreak` for which
 * single asset each version of the roster is least able to explain, and `windowOf` for
 * the seasons the value is heaviest in.
 *
 * THE TWO FINDINGS IT EXISTS FOR, and both are readings rather than recommendations:
 *
 *   `departingBreak` - the package sends the very asset this roster's OWN timeline
 *   already names as its odd one out. That is a thesis about what the deal is, not a
 *   claim that it fixes anything. `findTimelineBreak`'s own docstring is explicit that
 *   the named asset is very often the roster's best player and that holding one on
 *   purpose is a real strategy; copy built on this field must not contradict it.
 *
 *   `arrivingBreak` - the mirror: the package IMPORTS the asset the post-trade roster
 *   cannot explain. Same neutral register, same absence of a verdict. A deal can be
 *   worth making and still do this.
 *
 * @param {LeagueHistory} h
 * @param {number} rosterId
 * @param {TradeAsset[]} give
...
export function postTradeTimeline(h, rosterId, give, get, opts = {}) {
  ...
  const departingBreak =
    before.timelineBreak && outIds.has(String(before.timelineBreak.id))
      ? before.timelineBreak
      : null;
  const arrivingBreak =
    afterBreak && inIds.has(String(afterBreak.id)) ? afterBreak : null;
  /*
   * "AGAINST A CORE AT 4.7" - the rest of the roster's own weighted duration, with the
   * break asset taken out. Not `rosterDuration`, which INCLUDES the outlier and is
   * therefore dragged toward it: quoting that as the core would be comparing the asset
   * against a mean it is itself moving.
   */
  const coreWithout = (list, breakAsset) =>
    breakAsset
      ? coherenceOf(list.filter((a) => String(a.id) !== String(breakAsset.id)))
          .rosterDuration
      : null;
  return {
    before: {
