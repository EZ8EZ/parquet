...
function pool(n) {
  const m = new Map();
  for (let i = 1; i <= n; i++)
    m.set(`p${i}`, player(`p${i}`, `Player ${i}`, i));
  return m;
}
/** @returns {{ kind: "player", id: string, label: string }} */
const asset = (id, label = id) => ({ kind: "player", id, label });
/** @returns {{ kind: "pick", id: string, label: string }} */
const pick = (id) => ({ kind: "pick", id, label: `${id} pick` });
...
describe("what is deliberately NOT reported", () => {
  ...
  it("stays silent below the meaningful-gap threshold", () => {
    ...
    const index = convictionIndex(order, players);
    expect(
      convictionNotes({ give: [], get: [asset(order[10])] }, index),
    ).toEqual([]);
  });
  ...
  it("skips picks, which have no consensus player rank to disagree with", () => {
    ...
    const notes = convictionNotes(
      { give: [pick("2027-1-3")], get: [pick("2028-1-5")] },
      index,
    );
...
describe("ordering and caps", () => {
  ...
  it("honours an explicit threshold override", () => {
    const notes = convictionNotes({ give: [], get: [asset("p20")] }, index, {
  ...
  it("uses no em or en dashes in any generated prose", () => {
    ...
    const index = convictionIndex(
      [
        "p60",
        "p90",
        ...[...players.keys()].filter((id) => !["p60", "p90"].includes(id)),
      ],
      players,
    );
    const notes = convictionNotes(
