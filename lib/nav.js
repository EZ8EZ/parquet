export const ALL_SURFACES = [
  ...
  // ---------------------------------------------------------------- the app
  // "The Analyst" was listed here until 2026-08-19. It was shelved (SHELVED.md, S7)
  // when the app's direction settled on statistical reads derived from this league's
  // own history rather than LLM-generated prose. Everything its deterministic half
  // computed already renders on a surviving surface - the contradiction cards on
  // Home, the posture chips on a dossier, /plan's caveat - so nothing came with it.
  // Removing the entry is what removes the route from the drawer, /more, the
  // registry-driven smoke suite and the a11y sweep; there is no second list.
  {
    href: "/about",
    label: "What this is",
...
const ONWARD = {
  ...
  "/ledger": [
    ...
    // Was "/analyst - argue with someone who has read all of it". The ledger is a
    // pinned surface and dropping it to two ways out would have been a real loss, so
    // the slot went to the page that actually holds the ledger to account: /plan
    // leads with the stated-vs-revealed gap as a caveat whenever there is one
    // (lib/gameplan), which is the same fact the analyst opened with.
    { href: "/plan", why: "Does the plan still match what I wrote?" },
  ],
  "/recap": [
  ...
  "/trade/finder": [
    // Was "Adjust the package by hand", which described a link this page did not have:
    // every suggested package pointed at a bare `/trade` with nothing pre-filled, so
    // the registry was promising to carry a package the link dropped on the way. The
    // specific "open THIS package" button now exists on the package detail view, built
    // from `tradeHref`, which frees this generic entry to say what it actually does.
    { href: "/trade", why: "Start a package from scratch" },
  ...
  // /more deliberately has NO entry (VISION kill-list #7): its body IS the complete
  // index, and its two former steps (About, Methodology) were already listed in THE
  // APP group on the same screen. The no-dead-ends test exempts it by name.
  "/lab": [
...
export function homeNext(f) {
  ...
  if (f.contradicted) {
    // Pointed at /analyst until it was shelved (SHELVED.md, S7). The signal is NOT
    // dropped with it: /plan is already the baseline's first step, so a contradiction
    // now changes that step's REASON rather than adding a fourth destination, and it
    // changes it to the one thing /plan genuinely does differently when the gap
    // exists (lib/gameplan leads its caveats with the contradiction). A flag whose
    // only consequence was one dead link would have been dead config; a flag that
    // re-words the step a reader is going to take anyway is not.
    front.push({
      href: "/plan",
      why: "My record and my plan disagree. Which one is wrong?",
    });
  }
