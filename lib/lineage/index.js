...
/**
 * Who to credit as the drafter, for one lineage row rendered from one perspective.
 *
 * SHELVED.md S3: on the "gave" side ("which players did other managers take with
 * my old picks") the drafting team's name is the point of the row, so state it
 * every time - `usedByName` falls back to `toName` because most trades are single-hop
 * and `usedByName` is only set when the pick moved again after the last recorded
 * trade, before the draft. Everywhere else (no perspective, or "got" - you already
 * know you're the drafter) only surface the name when it disagrees with `toName`,
 * since otherwise it would just repeat what the hop arrow already showed.
 */
export function draftedByName(l, perspective) {
  if (!l?.resolved) return null;
  if (perspective === "gave") return l.usedByName ?? l.toName ?? null;
  return l.usedByName && l.usedByName !== l.toName ? l.usedByName : null;
}
