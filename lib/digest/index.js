 ...
 */
/**
 * The marker's sentinel for "no reading", distinct from every real clamped index
 * (0..100). RFI now refuses a score/band below four valued assets
 * (`MIN_ASSETS_FOR_FRAGILITY`, lib/metrics/fragility.js) rather than fabricating one,
 * and this codec has to be able to say that too: encoding a refusal as 0 would print
 * "fragility moved from 48 to 0, -48" the next time the reader visits, which is the
 * exact fabricated swing this app refuses to state anywhere else. TCI never refuses
 * (D102's fix keeps `tci: 0` as a real, if floor, number for its own zero-asset case),
 * so this only ever appears in the fragility slot in practice - but both slots share
 * one codec, so it is defined once rather than special-cased per metric.
 */
export const UNREAD_INDEX = -1;
export function encodeMarker(marker) {
  const rows = marker.metrics
    .slice(0, MAX_TRACKED_ROSTERS)
    .map(
      (r) => `${r.rosterId}:${encodeIndex(r.tci)}:${encodeIndex(r.fragility)}`,
    )
    .join("~");
...
export function parseMarker(raw) {
  ...
  if (parts[2].length > 0) {
    for (const chunk of parts[2].split("~")) {
      ...
      if (tci < 0 || tci > 100) return null;
      if (fragility !== UNREAD_INDEX && (fragility < 0 || fragility > 100))
        return null;
...
/** `clampIndex`, but a refusal passes straight through rather than clamping to 0. */
function encodeIndex(n) {
  return n === UNREAD_INDEX ? UNREAD_INDEX : clampIndex(n);
}
/**
...
export function buildDigest(h, input) {
  ...
  for (const row of metrics) {
    ...
    for (const metric of ["tci", "fragility"]) {
      ...
      const to = row[metric];
      // Either endpoint being unread is the same fact as having no baseline at all
      // (the `!before` continue above): there is no reading to compare, so there is
      // no honest "moved" to report. Without this, a roster crossing the refusal
      // floor either way would print a move built from a fabricated zero.
      if (from === UNREAD_INDEX || to === UNREAD_INDEX) continue;
      const delta = to - from;
...
export function currentMetrics(h) {
  ...
  return h.rosters
    .map((r) => {
      // `fragBy.get` is `null` for a roster RFI refuses to score (too few valued
      // assets) and `undefined` only if the roster is somehow missing from the pass
      // entirely - both mean "no reading", so both take the sentinel rather than the
      // `?? 0` this used to fall back to, which would have printed a fabricated
      // fragility of zero for a roster this app just declined to score.
      const frag = fragBy.get(r.rosterId);
      return {
        rosterId: r.rosterId,
        tci: clampIndex(tciBy.get(r.rosterId) ?? 0),
        fragility: frag != null ? clampIndex(frag) : UNREAD_INDEX,
      };
    })
