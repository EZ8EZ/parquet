...
describe("marker codec", () => {
  ...
  it("treats every malformed input as no marker instead of throwing", () => {
    const junk = [
      ...
      "v1.123.-1:2:3", // -1 is the fragility-only sentinel, not a roster id
      "v1.123.1:-1:3", // TCI never refuses, so -1 is not a legal TCI either
      "v1.123.1:2:-2", // below the sentinel, not the sentinel itself
  ...
  it("round-trips the unread sentinel rather than clamping it to a fabricated zero", () => {
    // A roster RFI refuses to score (lib/metrics/fragility.js, below four valued
    // assets) has to survive this codec as "no reading", not as a fragility of 0 -
    // 0 is a real, low, and entirely different claim.
    const m = marker(NOW, [{ rosterId: 1, tci: 60, fragility: UNREAD_INDEX }]);
    expect(parseMarker(encodeMarker(m))).toEqual(m);
  });
});
...
describe("buildDigest metric movement", () => {
  ...
  it("cannot invent movement crossing the refusal floor either way", () => {
    // A roster whose fragility read was unread at the marker's snapshot, or is
    // unread now, has no honest "moved from X to Y" - either endpoint being the
    // sentinel means there is nothing to subtract.
    const wasUnread = base.map((r) =>
      r.rosterId === 5 ? { ...r, fragility: UNREAD_INDEX } : r,
    );
    const nowReal = shifted(5, 0, 40);
    const d1 = buildDigest(h, input(marker(1, wasUnread), { metrics: nowReal }));
    expect(
      d1.moves.some((m) => m.rosterId === 5 && m.metric === "fragility"),
    ).toBe(false);
    const nowUnread = base.map((r) =>
      r.rosterId === 5 ? { ...r, fragility: UNREAD_INDEX } : r,
    );
    const d2 = buildDigest(h, input(marker(1, base), { metrics: nowUnread }));
    expect(
      d2.moves.some((m) => m.rosterId === 5 && m.metric === "fragility"),
    ).toBe(false);
  });
  it("caps the rendered list but still reports the true total", () => {
...
describe("currentMetrics", () => {
  ...
  it("reads the unread sentinel for a roster RFI refuses to score", () => {
    // Cloning rather than mutating `h1.rosters[0]` in place: `buildFixtureHistory`
    // hands back roster objects drawn from a module-level singleton
    // (lib/providers/fixture/index.js), so mutating one in place would leak into
    // every other `buildFixtureHistory()` call in this file/worker.
    const h1 = buildFixtureHistory();
    const victim = h1.rosters[0].rosterId;
    const stripped = {
      ...h1,
      rosters: h1.rosters.map((r) =>
        r.rosterId === victim ? { ...r, players: [] } : r,
      ),
      rostersById: new Map(
        h1.rosters.map((r) => [
          r.rosterId,
    ...
    const rows = currentMetrics(stripped);
    const row = rows.find((r) => r.rosterId === victim);
    expect(row.fragility).toBe(UNREAD_INDEX);
    // Nothing else on the same pass was touched by stripping one roster.
    for (const r of rows) if (r.rosterId !== victim) expect(r.fragility).toBeGreaterThanOrEqual(0);
  });
});
