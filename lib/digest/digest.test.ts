...
function metricRows(count: number, seed = 7): MetricRow[] {
  const rand = lcg(seed);
...
function marker(seenAt: number, metrics: MetricRow[] = []): DigestMarker {
  return { seenAt, metrics };
}

function pick(key: string, resolvedAt: number, ownerRoster: number): PickResolution {
  return {
    key,
...
function input(
  m: DigestMarker | null,
  extra: { metrics?: MetricRow[]; picks?: PickResolution[]; now?: number } = {},
) {
  return {
    marker: m,
    now: extra.now ?? NOW,
...
describe("buildDigest first visit", () => {
  ...
  it("still hands back a snapshot to record, or the next visit repeats the degrade", () => {
    ...
    expect(buildDigest(h, input(null, { metrics })).nextMetrics).toEqual(metrics);
  });
});
...
describe("buildDigest trades", () => {
  ...
  it("writes the viewer's own trades in second person and the rest neutrally", () => {
    ...
    for (const t of theirs) expect(t.description.startsWith("You ")).toBe(false);
  });
...
describe("buildDigest metric movement", () => {
  ...
  function shifted(rosterId: number, dTci: number, dFrag: number): MetricRow[] {
    return base.map((r) =>
      r.rosterId === rosterId
        ? {
            ...r,
  ...
  it("ignores movement below the threshold, so the panel is not noise", () => {
    const metrics = shifted(2, TCI_MOVE_THRESHOLD - 1, FRAGILITY_MOVE_THRESHOLD - 1);
  ...
  it("puts the viewer's own roster first however small its shift", () => {
    ...
    const metrics = base.map((r) => {
      if (r.rosterId === me) return { ...r, tci: Math.max(0, r.tci - TCI_MOVE_THRESHOLD) };
