...
export function isSafeOwnerId(id) {
...
const SEP = ".";
function signature(ownerId, secret) {
 ...
 */
export function signSeatToken(ownerId, secret) {
  if (!isSafeOwnerId(ownerId)) {
    throw new Error(
      `refusing to sign an unsafe owner id: ${JSON.stringify(ownerId)}`,
    );
  }
 ...
 * answer: no seat.
 */
export function verifySeatToken(token, secret) {
...
export const LEGACY_SEAT = { enforced: false, ownerId: null };
/** Turn a raw cookie value into a seat. Pure, so the whole matrix is unit-testable. */
export function resolveSeat(cookieValue, secret) {
 ...
 * before.
 */
export function writeAuthorId(seat, lensUserId) {
 ...
 */
export function viewAuthorId(seat, lensUserId) {
...
export function canCapture(seat, lensUserId) {
  return captureBlock(seat, lensUserId) === null;
}
export function captureBlock(seat, lensUserId) {
...
export function claimPath(ownerId, secret) {
...
export function claimUrl(ownerId, secret, origin) {
