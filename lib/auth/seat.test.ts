...
describe("resolveSeat", () => {
  ...
  it("enforces with a secret and no usable cookie, which is NOT the same as legacy", () => {
    for (const bad of [undefined, "", "forged"]) {
      expect(resolveSeat(bad, SECRET)).toEqual({ enforced: true, ownerId: null });
    }
  });
});
...
const legacy: Seat = LEGACY_SEAT;
const unclaimed: Seat = { enforced: true, ownerId: null };
const seated: Seat = { enforced: true, ownerId: EZ8 };
