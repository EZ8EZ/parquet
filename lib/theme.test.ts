...
describe("themeBootScript", () => {
  ...
  it("actually applies a stored theme, and ignores a bogus one", () => {
    ...
    const run = (stored: string | null, withMeta = true) => {
    ...
    expect(run("contrast").theme).toBe("contrast");
  ...
  it("moves the address-bar tint with the theme, and copes with the meta missing", () => {
    ...
    expect(run("contrast").chrome).toBe(THEME_CHROME.contrast);
    // A bogus theme changes neither the attribute nor the chrome.
