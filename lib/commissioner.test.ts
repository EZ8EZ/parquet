...
describe("getAuditLog", () => {
  ...
  it("gives trades a deep link into the trade web and nothing else does", () => {
    const withTxns: LeagueHistory = {
      ...
      transactions: [
        tx({ transactionId: "trade1", type: "trade", rosterIds: [1, 2] }),
        tx({ transactionId: "bigwaiver", type: "waiver", waiverBid: 25, rosterIds: [3] }),
      ],
    };
    const log = getAuditLog(withTxns);
  ...
  it("sorts newest first, tie-broken deterministically", () => {
    const withTxns: LeagueHistory = {
      ...h,
      transactions: [
        tx({ transactionId: "old", type: "trade", rosterIds: [1, 2], created: 1000 }),
        tx({ transactionId: "new", type: "trade", rosterIds: [1, 2], created: 2000 }),
      ],
    };
    const log = getAuditLog(withTxns);
...
describe("getStaleRosters", () => {
  ...
  it("flags a roster with unfilled starting slots by the real shortfall", () => {
    const withRosters: LeagueHistory = {
      ...h,
    ...
    expect(r.reasons.some((x) => x.kind === "empty-lineup-slots" && x.detail.includes("2"))).toBe(
      true,
    );
  });
  ...
  it("flags a roster whose last move is past the threshold, and not one just under it", () => {
    ...
    const stalePast: LeagueHistory = {
      ...h,
      rosters: [roster({ rosterId: 1, starters: fullStarters })],
      transactions: [
        tx({ rosterIds: [1], created: now - (STALE_DAYS_THRESHOLD + 1) * dayMs }),
      ],
    };
    ...
    expect(getStaleRosters(stalePast, { now }).find((s) => s.rosterId === 1)).toBeDefined();
    expect(getStaleRosters(recent, { now }).find((s) => s.rosterId === 1)).toBeUndefined();
  });
