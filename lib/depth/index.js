 ...
 * survives neither an export, a count, nor a screen reader (lib/refusal.js). This is
 * `SOURCE_GAP` and not `NO_RECORD`: Sleeper publishes a chart for this team and simply
 * has no row for this player, which is a fact about the payload and says nothing
 * whatever about him (D6).
 *
 * The withheld figure is the share of the team the chart DOES place, which is the one
 * number that tells a reader whether this gap is unusual - roughly one on-team player
 * in five has none, and a reader who does not know that reads an absence as meaningful.
 *
 * @param {DepthChart} chart
 * @param {string|null|undefined} playerId
