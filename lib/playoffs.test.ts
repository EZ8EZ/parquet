...
const g = (over: Partial<BracketGame>): BracketGame => ({
  matchId: 1,
  round: 1,
  placement: null,
  team1: null,
...
describe("placementsFrom", () => {
  it("gives the winner its place and the loser the next one", () => {
    const games = [
      g({ matchId: 9, round: 3, placement: 1, team1: 5, team2: 1, winner: 1, loser: 5 }),
      g({ matchId: 10, round: 3, placement: 3, team1: 2, team2: 4, winner: 2, loser: 4 }),
    ];
  ...
  it("contributes nothing for a placement game still undecided", () => {
    const games = [g({ placement: 1, team1: 3, team2: 7, winner: null, loser: null })];
    expect(placementsFrom(games).size).toBe(0);
...
describe("against the fixture corpus", () => {
  ...
  it("falls back cleanly when no bracket exists at all", () => {
    const bare = { ...h, brackets: new Map<string, BracketGame[]>() };
