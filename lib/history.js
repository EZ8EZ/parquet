 ...
 * The engines (strategy, dossier, gameplan, superlatives) are pure functions over
 * this object.
 */
import {
  activeLeagueId,
...
/**
 * @typedef {import('./providers/types.js').BracketGame} BracketGame
 * @typedef {import('./providers/types.js').LeagueDetail} LeagueDetail
 * @typedef {import('./providers/types.js').LeagueUser} LeagueUser
 * @typedef {import('./providers/types.js').Matchup} Matchup
 * @typedef {import('./providers/types.js').Player} Player
 * @typedef {import('./providers/types.js').Roster} Roster
 * @typedef {import('./providers/types.js').TradedPick} TradedPick
 * @typedef {import('./providers/types.js').Transaction} Transaction
 */
/**
 * @typedef {Object} Annotation
 * @property {string} transactionId
 * @property {string} ownerId Durable author identity — a Sleeper platform user id
 *   (a PRINCIPAL's ownerId, D22), never a roster id.
 ...
 * @typedef {Matchup & { season: string }} HistoryMatchup
 */
/**
 * @typedef {Object} Me
 * @property {string} userId
 * @property {number|null} rosterId
 * @property {string} displayName
 * @property {string|null} teamName
 */
/**
 * The single corpus object every derivation engine (strategy, dossier, gameplan,
 * superlatives) consumes as a pure function's input. See this file's header for
 * what's DB-free vs DB-backed.
 * @typedef {Object} LeagueHistory
 * @property {string} provider
 * @property {LeagueDetail} currentLeague
 * @property {LeagueDetail[]} chain oldest -> newest
 * @property {LeagueUser[]} users
 * @property {Map<string, LeagueUser>} usersById
 * @property {Map<number, Roster>} rostersById
 * @property {Roster[]} rosters
 * @property {Map<string, Player>} players
 * @property {Transaction[]} transactions chronological asc
 * @property {TradedPick[]} tradedPicks current-league snapshot: who owns which future pick right now
 * @property {TradedPick[]} tradedPicksHistory pick movement across every season in the chain
 * @property {HistoryMatchup[]} matchups
 * @property {Map<string, BracketGame[]>} brackets winners bracket per season
 * @property {Map<string, Annotation>} annotations keyed by `annotationKey(transactionId, ownerId)`
 * @property {Me} me the LENS: whose public data this render is about
 * @property {string|null} [authorId] the SEAT: whose private authorship this request may read/edit (see `viewerAuthorId`)
 * @property {number} currentSeasonYear
 */
/**
 ...
 * against, and the one every downstream engine (ledger, strategy, recap) inherits
 * by going through it.
 *
 ...
 * @param {LeagueDetail[]} chain
 * @returns {Promise<HistoryMatchup[]>}
 */
 ...
 *
 * Typed narrowly to just the one method this function calls, rather than the full
 * `LeagueProvider` interface: the concrete provider classes (Sleeper/CSV/fixture)
 * are not themselves JSDoc-annotated yet (out of this pass's scope — see D88), so
 * their inferred shapes don't structurally match `LeagueProvider` in every field,
 * and this function only ever needed `getBracket` anyway.
 * @param {{ name: string, getBracket?: (leagueId: string, kind: import('./providers/types.js').BracketKind) => Promise<BracketGame[]> }} provider
 * @param {LeagueDetail[]} chain
 * @returns {Promise<Map<string, BracketGame[]>>}
 */
...
/**
 * The identity-independent piece of the corpus — everything `LeagueHistory` has
 * except `me`/`authorId`, plus the default "me" username resolution `getLeagueHistory`
 * layers a lens onto.
 * @typedef {Omit<LeagueHistory, 'me'|'authorId'> & { defaultMeUserId: string }} Corpus
 */
/**
 * @typedef {Object} CorpusSlot
 * @property {Promise<Corpus>} promise
...
async function assembleCorpus() {
  ...
  // what gets published is the GAP: the `coalesced-` id this function mints is what
  // `isCommissionerExecuted()` reads, and every surface that shows one of these deals
  // says the same thing about it ("Pick record missing") rather than quietly rendering
  // it as an ordinary trade (D100). A `unrecordedPickMoves()` helper was cited here for
  // three rounds as surfacing them "separately"; it never had a caller and is gone.
...
/**
 * @typedef {Object} GetLeagueHistoryOpts
 * @property {boolean} [fresh] force a fresh corpus assembly, bypassing the TTL cache
 * @property {number|null} [meRosterId] override the "viewing as" roster instead of reading the cookie (tests)
 */
/**
 * @param {GetLeagueHistoryOpts} [opts]
 * @returns {Promise<LeagueHistory>}
 */
