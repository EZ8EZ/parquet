...
async function assembleCorpus() {
  ...
  // assembly done, these five ran in series (collectTransactions dominates - it is
  // already internally fanned out over 5 seasons x 25 weeks - collectTradedPicks/
  // loadAnnotations/loadMatchups/loadBrackets add their own serial time on TOP of
  // that, for no reason: none of them wait on collectTransactions's result either).
  // Running them together bounds the group by the slowest one instead of the sum.
