...
function hopOf(ctx, m, nameOf) {
  ...
  return {
    ...
    parties: m.parties,
    // Was this deal executed by the commissioner by hand? `/deals` has marked this
    // since D19 and the rail did not, so the one surface that draws a single asset's
    // whole history was also the one that could not say "the pick side of this hop was
    // never recorded". Defaults to false for a hand-built test context with no flag,
    // which is the safe direction: a missing flag prints no claim rather than a
    // wrong one.
    commissionerExecuted: m.commissionerExecuted === true,
  };
}
