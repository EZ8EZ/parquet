...
describe("refusalSentence", () => {
  it("leads with the code, which is what makes it learnable across surfaces", () => {
    const s = refusalSentence(refusal("NO_RECORD", "Nothing is priced here."));
    expect(s.startsWith("NO_RECORD: ")).toBe(true);
  ...
  it("says nothing about a withheld figure when none was computable", () => {
    ...
    expect(s).toBe("UNSCHEDULED: Nobody has scheduled it.");
  });
});
describe("refusalShort", () => {
  it("is the code alone, for a cell with no room for a sentence", () => {
    expect(refusalShort(refusal("SPLIT_ROSTER", "x"))).toBe("SPLIT_ROSTER");
  });
