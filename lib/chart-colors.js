 ...
 * dark. The bottom two steps are under 3:1 on every ground, and no floor fixes it:
 * the weakest step only clears 3:1 at
 ...
 * graphical object has to clear. Measured contrast of the shipped pair, against both
 * grounds this app has - and against each theme's card surface as well, since a
 * chart sits on a card more often than on the page:
 *
 *                    Paper bg / surface   Dark bg / surface
 *   low   #d2569f       3.43   3.74         5.19   4.71
 *   high  #4d9221       3.51   3.82         5.08   4.61
 *
 * All eight clear 3:1; the tightest is 3.43. None clears 4.5:1 on both, which
 * is rule 4.
 *
 ...
 *
 * MEASURED, AND IT HAS A CEILING ON WHAT IT MAY CARRY: composited on `--color-surface`
 * this is 2.59:1 on dark and 2.09:1 on paper. Both are under 3:1, which is the same
 * floor rule 2 applies to the magnitude ramp - so it is legitimate for a mark whose
 * value is ALSO printed or carried by a length, and it is NOT legitimate where the
 * mark's own visibility is the datum. It is a border token and it reads like one.
 *
 * If a mark's POSITION is the only statement of its value, use `CHART_MARK`.
 */
export const CHART_NEUTRAL = "var(--color-border-strong)";
/**
 * A context mark whose POSITION IS THE DATUM - a dot on a scale with no number printed
 * beside it, the quiet half of a dumbbell, a tick a reader is expected to locate.
 *
 * `--color-secondary` rather than `--color-border-strong`, and the reason is measured:
 * 6.09:1 on paper and 5.74:1 on dark against `--color-surface`, against 2.09 and 2.59
 * for the border token. `globals.css` already reserves `secondary` BY JOB for "anything
 * carrying a datum" and `border-strong` for emphasis borders, so this is the token that
 * was always correct for a mark. The borrowed border token merely looked correct on the
 * dark theme, which is where the gap between the two is smallest.
 *
 * Still not the subject: `CHART_ACCENT` stays the one-per-chart subject ink, and this is
 * for the marks a reader compares that subject AGAINST.
 */
export const CHART_MARK = "var(--color-secondary)";
/** The subject. One per chart at most - the moment two things are accent, neither is. */
export const CHART_ACCENT = "var(--color-accent)";
/**
 * THE FIELD - court blue, the second hue, adopted by VISION.md M4 with exactly one
 * sentence of meaning: **gold = yours, blue = the field** (the market, consensus,
 * everyone-else's side of a comparison). It rides the existing `info` token rather
 * than adding a fifth semantic hue, per M4's own instruction, so it inherits the
 * measured per-theme values that token already carries (5.53:1 on its own wash dark,
 * 5.31:1 paper - see globals.css).
 *
 * THE RULE, Teenage-Engineering strict: a mark gets this hue only if its caption can
 * be written as "this is the field's side of a comparison against you." Distribution
 * strips' peer ticks, the consensus side of a rank gap, the incoming side of a trade.
 * Never decorative, never on a chart with no "you" in it - such a chart keeps
 * CHART_MARK/CHART_NEUTRAL. And never the sole encoding (D47): every field mark still
 * carries its value by position or length first.
 */
export const CHART_FIELD = "var(--color-info)";
/**
