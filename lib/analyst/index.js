...
import { getAllDossiers } from "../dossier/index.js";
import { getPrincipals } from "../principals.js";
import {
  describeTransaction,
  describeTradeForRoster,
} from "../derive/describe.js";
import { valuePlayer } from "../valuation/index.js";
import { leagueTierLabel } from "../rankings/leagueTiers.js";
import { ADVERSARIAL_REMINDER, ANALYST_SYSTEM_PROMPT } from "./system-prompt.js";
import { traceLLMRun } from "../observability/trace.js";
/** Build the compact text corpus injected into the analyst prompt. */
export function buildCorpus(h, principals) {
  const report = getStrategyReport(h, principals);
  const p = report.profile;
  const rosterId = h.me.rosterId;
  const lines = [];
  lines.push(
    `# YOU: ${h.me.teamName ?? h.me.displayName} (${h.me.displayName})`,
  );
  lines.push(
    `League: ${h.currentLeague.name}, ${h.chain.length} seasons (${h.chain.map((c) => c.season).join(", ")}), ${h.currentLeague.totalRosters} teams.`,
  );
  lines.push("");
  lines.push(`## REVEALED STRATEGY (derived from your transactions)`);
  lines.push(
    `Trades: ${p.trades} (${p.tradesInitiated} you initiated). Waivers/FA: ${p.waivers + p.freeAgents}.`,
  );
  lines.push(
    `Pick capital: net ${p.picks.net} (firsts: +${p.picks.firstsAcquired}/-${p.picks.firstsSpent}).`,
  );
  if (p.acquisitions.avgAge != null)
    lines.push(
      `Avg acquisition age: ${p.acquisitions.avgAge}. Age trend by season: ${p.acquisitions.ageBySeason.map((a) => `${a.season}:${a.avgAge}`).join(", ")}.`,
    );
  lines.push(
    `Revealed posture by season: ${p.postureBySeason.map((x) => `${x.season}:${x.posture}`).join(", ")}.`,
  );
  if (p.afterLoss)
    lines.push(
      `Self-initiated trades after a loss: ${p.afterLoss.afterLoss}/${p.afterLoss.total}.`,
    );
  ...
  if (annotated.length === 0) {
    ...
    for (const t of annotated) {
      ...
      const desc =
        rosterId != null && t.type === "trade"
          ? `you ${describeTradeForRoster(h, t, rosterId)}`
          : describeTransaction(h, t);
      lines.push(`- [${t.season}] ${desc}`);
      lines.push(
        `  Your reasoning: "${ann.reasoning}"${ann.posture ? ` (posture: ${ann.posture})` : ""}`,
      );
    }
  }
  lines.push("");
  ...
  if (myTrades.length) {
    ...
    for (const t of myTrades.slice(-25)) {
      lines.push(
        `- [${t.season} wk${t.week}] you ${describeTradeForRoster(h, t, rosterId)}${myAnnotation(h, t.transactionId) ? "" : "  (UNANNOTATED)"}`,
      );
    }
    lines.push("");
  }
  ...
  if (rosterId != null) {
    ...
    if (roster) {
      ...
      for (const v of valued.slice(0, 12))
        lines.push(
          `- ${v.name} (age ${v.age ?? "?"}) - ${v.value} [${v.tier}]`,
        );
      lines.push("");
    }
  }
  ...
  for (const d of getAllDossiers(h, principals)) {
    lines.push(
      `- ${d.profile.displayName}${d.profile.teamName ? ` (${d.profile.teamName})` : ""}: ${d.tags.join(", ") || "no strong tell"}. ${d.read}`,
    );
  }
  return lines.join("\n");
}
export async function runAnalyst(h, question, prior = []) {
  ...
  try {
    ...
    return {
      text: text || rulesFallback(h, question, principals),
      mode: "llm",
      model,
    };
  } catch (err) {
 ...
 */
export function rulesFallback(h, question, principals) {
  ...
  if (report.contradictions.length) {
    out.push(
      `First, the uncomfortable part: ${report.contradictions[0].narrative}`,
    );
    out.push("");
  } else {
  ...
  if (
    ...
    out.push(
      `Your revealed posture by season: ${post.map((p) => `${p.season}=${p.posture}`).join(", ") || "insufficient data"}.`,
    );
  }
  out.push("What your record actually shows:");
  for (const f of report.findings) out.push(`• ${f}`);
  if (!report.findings.length)
    out.push("• Not enough transaction history yet to draw firm patterns.");
  out.push("");
  out.push(
