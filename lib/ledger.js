...
import { describeTradeForRoster, describeTransaction } from "./derive/describe.js";
 ...
 */
export function isFaabLeague(h) {
 ...
 */
function contestedWaiverIds(h) {
  const rostersByKey = new Map();
  for (const t of h.transactions) {
    if (t.type !== "waiver") continue;
    for (const [playerId, rosterId] of Object.entries(t.adds)) {
      const key = `${t.season}|${t.week}|${playerId}`;
      (rostersByKey.get(key) ?? rostersByKey.set(key, new Set()).get(key)).add(
        rosterId,
      );
    }
  }
 ...
 * need it.
 */
export function buildIsNotable(h) {
 ...
 */
export function notableWaiverLabel(h) {
 ...
 */
export function getLedgerEntries(h, principals) {
 ...
 */
export function newestToCapture(entries) {
  let best = null;
...
export function getLedgerSummary(h, principals, now = Date.now()) {
