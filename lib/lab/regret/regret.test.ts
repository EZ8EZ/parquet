...
import { buildRegretLedger, scoreGame, type LedgerInput } from "./index";
...
function game(stats: Record<string, number>, date = "2025-11-17") {
...
describe("the regret ledger", () => {
  const games = new Map([
    ...
    ["A", new Map([[1, [game({ pts: 60 }), game({ pts: 100, bonus_pt_40p: 1, bonus_pt_50p: 1 })]]])],
    ["B", new Map([[1, [game({ pts: 40 })]]])],
    ["C", new Map([[1, [game({ pts: 20 })]]])],
    // D never played that week.
    ["D", new Map<number, ReturnType<typeof game>[]>([[1, []]])],
  ]);
  const names = new Map([
    ["A", "Ada"],
    ["B", "Bo"],
  ...
  it("takes each player's BEST game once, never a player twice", () => {
    ...
    expect(new Set(w.bestSeven.map((g) => g.playerId)).size).toBe(w.bestSeven.length);
  ...
  it("refuses to name the game when two nights scored the same", () => {
    const twin = new Map([
      ["A", new Map([[1, [game({ pts: 60 }, "2025-11-17"), game({ pts: 60 }, "2025-11-19")]]])],
    ]);
    const l = buildRegretLedger(
      input({
        games: twin,
  ...
  it("tags playoff weeks and totals the season", () => {
    const wk = (week: number, pts: number) => ({
      week,
      starters: ["A", "0", "0", "0", "0", "0", "0"],
    ...
    const l = buildRegretLedger(
      input({
        games: new Map([
          ["A", new Map([[1, [game({ pts: 60 })]], [21, [game({ pts: 80 })]]])],
        ]),
        playerNames: names,
