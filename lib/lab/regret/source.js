...
const RawWeekly = z.union([
  z.record(z.string(), z.array(RawGame)),
  z.array(z.unknown()),
]);
...
async function memo(cache, key, load) {
...
export async function loadPlayerSeason(playerId, season) {
...
export async function loadPlayerSeasons(playerIds, season, concurrency = 8) {
  const out = new Map();
