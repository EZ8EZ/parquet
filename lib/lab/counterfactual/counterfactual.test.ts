...
import { buildDraftIndex, invalidateDraftIndex, type DraftIndex } from "../../lineage";
...
async function fixture(): Promise<{ h: LeagueHistory; index: DraftIndex }> {
...
describe("the counterfactual roster", () => {
  ...
  it("never lets a trade put a player into the counterfactual", async () => {
    ...
    for (const sd of index.bySeason.values()) {
      for (const p of sd.picks) {
        if (p.playerId && sd.draft.slotToRosterId[p.draftSlot] === 4) drafted.add(p.playerId);
      }
    }
    const added = new Set<string>();
    for (const t of h.transactions) {
      if (t.type === "trade") continue;
      for (const [pid, to] of Object.entries(t.adds)) if (to === 4) added.add(pid);
    }
  ...
  it("keeps a player this roster drafted and later traded away", async () => {
    ...
    for (const t of h.transactions) {
      if (t.type === "trade") continue;
      for (const [pid, from] of Object.entries(t.drops)) droppedBy.add(`${pid}|${from}`);
    }
    let owner = -1;
    let playerId = "";
  ...
  it("trims to the roster size actually held, and reports what it left behind", async () => {
    ...
    for (const r of h.rosters) {
      ...
      expect(c.overflow).toBe(
        c.players.filter((p) => p.priced && p.value > 0).length - keptRows.length,
      );
    }
  });
  ...
  it("excludes players with no NBA team from the totals rather than scoring them 0", async () => {
    ...
    const withTeams: LeagueHistory = {
      ...h,
      players,
      transactions: [...h.transactions, move("2024", 1, { [stranded]: 2 }, {})].sort(
    ...
    expect(c.players.filter((p) => p.kept).map((p) => p.playerId)).not.toContain(stranded);
  });
  ...
  it("reports overlaps rather than resolving them", async () => {
    ...
    expect(counterfactualOverlaps(withMoves, index).get(shared)).toBeUndefined();

    const bothKeep: LeagueHistory = {
      ...h,
      transactions: [
        ...h.transactions,
        move("2023", 1, { [shared]: 5 }, {}),
  ...
  it("degrades to an empty counterfactual with no draft data", async () => {
    ...
    const empty: DraftIndex = { supported: false, bySeason: new Map() };
...
describe("describeCounterfactual", () => {
  it("states the delta without grading it, for every roster in the league", async () => {
    ...
    for (const r of h.rosters) {
      const lines = describeCounterfactual(buildCounterfactual(h, r.rosterId, index));
...
describe("pickCapital's ownership lens", () => {
  ...
  it("partitions every tracked pick across the league exactly once, both ways", async () => {
    const { h } = await fixture();
    for (const ownership of ["held", "original"] as const) {
