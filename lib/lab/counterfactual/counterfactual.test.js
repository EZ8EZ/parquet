...
import { buildDraftIndex, invalidateDraftIndex } from "../../lineage/index.js";
...
describe("the counterfactual roster", () => {
  ...
  it("excludes players with no NBA team from the totals rather than scoring them 0", async () => {
    const { h, index } = await fixture();
    // BOTH BRANCHES, and the fixture is no longer the one that supplies the first:
    // it carries real NBA teams now (it has to - `/depth/[team]` reads them), so the
    // "provider tracks no teams at all" case is built by stripping them here rather
    // than by relying on the fixture to be incomplete. That case still matters: the
    // CSV provider's `team` column is optional.
    const teamless = {
      ...h,
      players: new Map(
        [...h.players].map(([id, p]) => [id, { ...p, team: null }]),
      ),
    };
    expect(corpusTracksNbaTeams(teamless)).toBe(false);
    expect(buildCounterfactual(teamless, 1, index).teamCheckAvailable).toBe(
      false,
    );
    expect(buildCounterfactual(teamless, 1, index).unpriced).toHaveLength(0);
    // And the fixture as it now ships does fire the check.
    expect(corpusTracksNbaTeams(h)).toBe(true);
    // Put every player on one team and strand exactly one without.
