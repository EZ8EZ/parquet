...
describe("league board url", () => {
  ...
  it("degrades an unknown, empty or hand-mangled param to the default", () => {
    ...
    for (const s of ["", "?", "?board=", "?board=nonsense", "?board=duration", "?other=1", "?%%%"]) {
  ...
  it("preserves any other params already on the URL", () => {
    ...
    expect(boardSearch("?board=fragility&focus=abc", "windows")).toBe("?focus=abc");
  });

  it("round-trips", () => {
    for (const b of ["windows", "fragility"] as const) {
