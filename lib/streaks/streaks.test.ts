...
import { humanDays, liveStreaks } from "./index";
...
function world(transactions: Transaction[], players: string[]): LeagueHistory {
  const roster = { ...base.rosters[0], rosterId: 1, players } as Roster;
  return {
    ...base,
    rosters: [roster],
...
const find = (h: LeagueHistory, id: string, now = NOW) =>
...
describe("hold streaks", () => {
  ...
  it("only calls the two-year count a floor when an unknown start could miss it", () => {
    const withPlayers = (recordAgeDays: number) =>
      world(
        [tx({ transactionId: "a", created: NOW - recordAgeDays * DAY, adds: { p1: 1 } })],
        ["p1", "p2"],
      );
...
describe("trade cadence streaks", () => {
  ...
  it("holds a season run at risk while the current season is still open", () => {
    ...
    const h = world([tx({ transactionId: "a", created: NOW - 400 * DAY, season: previous })], []);
  ...
  it("marks the season run live once the current season has a trade", () => {
    const seasons = base.chain.map((l) => l.season);
    const current = seasons[seasons.length - 1];
    const h = world([tx({ transactionId: "a", created: NOW - 5 * DAY, season: current })], []);
    const s = find(h, "season-run")!;
    expect(s.state).toBe("growing");
...
describe("nothing here restates a Superlative", () => {
  it("every streak is present-tense and carries a live state", () => {
    ...
    for (const s of streaks) {
      ...
      expect(s.label.toLowerCase()).not.toMatch(/\b(most|fewest|best|worst|ever)\b/);
    }
  });
});
