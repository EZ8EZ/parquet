 ...
 * testable.
 */
import { holdingSpans, involves } from "../derive/manager.js";
 ...
 */
export function liveStreaks(h, rosterId, opts = {}) {
  ...
  const nameOf = (pid) => h.players.get(pid)?.fullName ?? `Player ${pid}`;
    ...
    out.push({
      ...
      detail: matured.length
        ? `${matured
            ...
            .join(
              ", ",
            )}${matured.length > 3 ? ` and ${matured.length - 3} more` : ""}.`
  ...
  for (let i = seasons.length - 1; i >= 0; i--) {
    ...
    else if (seasons[i] === currentSeason)
      continue; // still open, not yet a miss
    else break;
  }
  if (run > 0) {
