...
import { buildPrincipals, tenureSeasons } from "./principals.js";
function user(id, name, team) {
  return {
    userId: id,
    displayName: name,
    avatar: null,
...
describe("succession detection", () => {
  ...
  it("scopes tenure seasons to one roster", () => {
    expect([...tenureSeasons(idx.byOwnerId.get("b"))]).toEqual([
      "2022",
      "2023",
    ]);
