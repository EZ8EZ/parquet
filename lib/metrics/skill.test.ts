...
import {
  ...
  tradeValueProfiles,
  type RawDraftPick,
  type SeasonStartRate,
} from "./skill";

// ------------------------------------------------------------------ start rate

const season = (s: string, fpts: number, ppts: number): SeasonStartRate => ({
  season: s,
  fpts,
  ppts,
  startRate: fpts / ppts,
...
describe("start rate (lock-in management)", () => {
  ...
  it("keeps leftOnBench consistent with the headline rate", () => {
    const p = foldStartRate("u1", 1, [season("2023", 3000, 3400), season("2024", 2000, 2100)]);
...
function pick(pickNo: number, rosterId: number, playerId: string): RawDraftPick {
  return { pickNo, rosterId, playerId, playerName: playerId, round: 1, isKeeper: false };
}

/** Four players worth 100, 80, 60, 40 taken in that order by rosters 1..4. */
const VALUES: Record<string, number> = { best: 100, good: 80, ok: 60, bad: 40 };
const valueOf = (id: string) => VALUES[id] ?? 0;
const nameOf = (id: string) => id;
const ownerOf = (_s: string, rosterId: number) => `owner${rosterId}`;
...
describe("gradeDraft", () => {
  it("scores 1 for taking the best asset left and 0 for taking the worst", () => {
    ...
    const g = gradeDraft(
      "2024",
      [pick(1, 1, "best"), pick(2, 2, "bad"), pick(3, 3, "good"), pick(4, 4, "ok")],
      valueOf,
      nameOf,
      ownerOf,
    );
  ...
  it("regret is never positive: you cannot beat the best available", () => {
    const g = gradeDraft(
      "2024",
      [pick(1, 1, "ok"), pick(2, 2, "best"), pick(3, 3, "good"), pick(4, 4, "bad")],
      valueOf,
      nameOf,
      ownerOf,
    );
  ...
  it("gives the first pick a perfect capture but no slot surplus", () => {
    const g = gradeDraft(
      "2024",
      [pick(1, 1, "best"), pick(2, 2, "good"), pick(3, 3, "ok"), pick(4, 4, "bad")],
      valueOf,
      nameOf,
      ownerOf,
    );
  ...
  it("rewards a late pick on a player the class left behind", () => {
    ...
    const g = gradeDraft(
      "2024",
      [pick(1, 1, "bad"), pick(2, 2, "ok"), pick(3, 3, "good"), pick(4, 4, "best")],
      valueOf,
      nameOf,
      ownerOf,
    );
  ...
  it("value ranks are a permutation, so ties cannot double-count", () => {
    const flatish = (id: string) => (id === "best" ? 100 : 50);
    const g = gradeDraft(
      "2024",
  ...
  it("degrades on tiny inputs", () => {
    ...
    expect(gradeDraft("2024", [pick(1, 1, "best")], valueOf, nameOf, ownerOf)).toEqual([]);
  });
});
...
describe("foldDraftCapture", () => {
  ...
  it("weights a big opportunity above a small one", () => {
    const bigThenSmall = foldDraftCapture([
      { ...graded[0], ownerId: "x", value: 100, bestAvailable: 100, worstAvailable: 0, capture: 1, regret: 0 },
      { ...graded[0], ownerId: "x", value: 0, bestAvailable: 10, worstAvailable: 0, capture: 0, regret: -10 },
    ]);
    const smallThenBig = foldDraftCapture([
      { ...graded[0], ownerId: "y", value: 10, bestAvailable: 10, worstAvailable: 0, capture: 1, regret: 0 },
      { ...graded[0], ownerId: "y", value: 0, bestAvailable: 100, worstAvailable: 0, capture: 0, regret: -100 },
    ]);
  ...
  it("takes the steal and the reach from rookie drafts, not the startup", () => {
    ...
    const rookie = gradeDraft(
      "2024",
      [pick(1, 1, "bad"), pick(2, 1, "ok"), pick(3, 1, "best"), pick(4, 1, "good")],
      valueOf,
      nameOf,
      () => "owner1",
      false,
  ...
  it("normalises surplus by depth so a shallow draft can win", () => {
    ...
    const shallow = { ...graded[0], ownerId: "shallow", slotSurplus: 5, slotSurplusRate: 5 / 10, draftSize: 10, isStartup: false };
    const deep = { ...graded[0], ownerId: "deep", slotSurplus: 8, slotSurplusRate: 8 / 40, draftSize: 40, isStartup: false };
    const f = foldDraftCapture([shallow, { ...shallow, slotSurplusRate: 0, slotSurplus: 0 }, deep, { ...deep, slotSurplusRate: 0, slotSurplus: 0 }]);
  ...
  it("separates the pool-relative extremes from the slot-relative ones", () => {
    const p = folded.get("owner1")!;
    expect(p.steal).not.toBeNull();
    expect(p.steal!.slotSurplus).toBeGreaterThanOrEqual(p.bust?.slotSurplus ?? -Infinity);
  });
...
function principalsFor(h: ReturnType<typeof buildFixtureHistory>) {
