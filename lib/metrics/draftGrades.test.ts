...
import { gradeDraft, type RawDraftPick } from "./skill";
...
const meta = (over: Partial<{ draftId: string; rounds: number; teams: number; totalPicks: number; isStartup: boolean }> = {}) => ({
  draftId: "d1",
  rounds: 1,
  teams: 4,
  totalPicks: 4,
...
describe("foldSeasonDraftGrade", () => {
  ...
  it("suppresses steal and bust on a startup season, even though pool capture still works", () => {
    const graded = gradeDraft(
      "2022",
      [pick(1, 1, "bad"), pick(2, 1, "best"), pick(3, 1, "ok"), pick(4, 1, "good")],
      valueOf,
      nameOf,
      ownerOf,
      true,
