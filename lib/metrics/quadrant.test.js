...
describe("quadrant copy", () => {
  ...
  it("never calls the low-fragility half safe, healthy or resilient", () => {
    const banned =
      /\b(safe|safest|healthy|resilient|sturdy|insulated|bulletproof)\b/i;
    for (const k of ["agreedSpread", "splitSpread"]) {
      const q = QUADRANTS[k];
  ...
  it("says out loud that a spread-and-split roster may simply be empty", () => {
    expect(QUADRANTS.splitSpread.thesis).toMatch(
      /not because it is insulated/i,
    );
  });
...
describe("placeLabels", () => {
  ...
  it("steps a label around a neighbouring dot, not just a neighbouring label", () => {
    ...
    expect(
      placeLabels([{ x: 100, y: 100 }], { ...opts, radii: [6] })[0].side,
    ).toBe("right");
  });
...
describe("buildQuadrantView", () => {
  ...
  it("breaks a TCI tie on fragility, then on roster id, so the order is total", () => {
    ...
    expect(buildQuadrantView(tied, frag, null).points.map((p) => p.n)).toEqual([
      1, 2, 3,
    ]);
  });
