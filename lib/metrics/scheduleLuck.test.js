...
import {
  ...
  seasonScheduleLuck,
} from "./scheduleLuck.js";
/** A weekly matchup row. `season` defaults to "2024" to keep test tables terse. */
function row(week, rosterId, matchupId, points, season = "2024") {
...
describe("allPlaySeasonLuck", () => {
  ...
  it("returns null for a season with no games", () => {
    ...
    expect(
      allPlaySeasonLuck([row(1, 2, 1, 100), row(1, 3, 1, 90)], 1, "2024"),
    ).toBeNull();
  });
...
describe("aggregateSeasonLuck", () => {
  ...
  it("returns null for a season with no games", () => {
    expect(
      aggregateSeasonLuck(
        "2024",
        settings({ wins: 0, losses: 0, ties: 0, fpts: 0, fptsAgainst: 0 }),
      ),
    ).toBeNull();
  });
  it("returns null when no points were recorded (an unplayed/future season)", () => {
    ...
    expect(
      aggregateSeasonLuck(
        "2024",
        settings({ wins: 1, losses: 0, ties: 0, fpts: 0, fptsAgainst: 0 }),
      ),
    ).toBeNull();
  });
});
