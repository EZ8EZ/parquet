...
import { refusal, refusalSentence } from "../refusal.js";
// ---------------------------------------------------------------------------------
...
/**
 * Fewest valued assets a fragility score/band may be published from.
 *
 * MECHANICAL, not a judgement call, and measured rather than reasoned about. At or
 * below `LOO_TOP_K` assets, "your three most load-bearing players carry X% of
 * startable value" is measuring the roster against itself - there is no roster left
 * outside the top three to compare it to, because the top three ARE the roster.
 * Verified on every one of the fixture league's fourteen real rosters, stripped to
 * their own top 1, 2 or 3 assets by value (and on a synthetic one-dominant-asset-plus-
 * three-negligible one): `topKDamageShare` reads exactly 1.000 and `concentrationScore`
 * reads exactly 100 in every single case, proven rather than just observed - even the
 * most EVEN possible split at three assets clears `CONCENTRATION_REF` (a reference
 * calibrated for this league's real 15-to-32-asset rosters, not for three of them).
 * Between them, LOO and concentration are 0.8 of this index's weight, and both are
 * pinned to their ceiling regardless of who the assets are - so `raw` has a mechanical
 * floor of 80 at this sample size no matter what is on the roster. It is not always
 * EXACTLY 80: the remaining 0.2 (exposure) is real signal and does move when the top 1
 * to 3 assets happen to carry age or injury risk - two of the fourteen fixture rosters
 * read 85-88 rather than 80 at n=2/3 for exactly that reason - but 80% of the number's
 * weight cannot tell any of these rosters apart at this sample size, which is the
 * property that matters.
 *
 * `LOO_TOP_K + 1` is the floor because a genuine fourth asset outside the top three
 * exists at that point, and the number starts moving with composition again - measured
 * spread 66.6 to 84.1 raw across the fourteen rosters' own top-4, a real range rather
 * than a repeated constant. It is still calibrated for the 15-to-32-asset rosters this
 * league actually carries and can still read high on a genuinely lopsided four-asset
 * set (the synthetic one-star-plus-three-scrubs case above reads 80.0 again at n=4,
 * correctly - a real roster where three-quarters of it is nearly worthless), which is
 * why this floor fixes the ARTEFACT and does not promise a well-calibrated number,
 * only a non-degenerate one.
 *
 * Checked against the real fixture league before being committed: every one of its
 * fourteen rosters carries 16 valued assets, so this floor changes nothing observable
 * on the live product today - the fix is latent, the same status D102 recorded for
 * TCI's mirror-image bug.
 */
export const MIN_ASSETS_FOR_FRAGILITY = LOO_TOP_K + 1;
/**
...
export function leagueFragility(h, cfg = VALUATION_CONFIG) {
  ...
  //
  // A REFUSED ROSTER'S SCORE NEVER JOINS THE LADDER. `scoreFragility` still runs on
  // every roster above (it is cheap, and the raw number is a useful internal sort key
  // even when it will not be published), but a roster below `MIN_ASSETS_FOR_FRAGILITY`
  // reads a number that is an artefact of its own count, not of the league - and if
  // that number were in the ladder, it would move every OTHER roster's percentile and
  // band off a data point nobody is willing to publish.
  const ladder = fragilityLadder(
    scored
      .filter((s) => s.assets.length >= MIN_ASSETS_FOR_FRAGILITY)
      .map((s) => s.score.raw),
  );
  return scored
    .map((s) => ({
    ...
    .sort((a, b) => {
      const fa = a.profile.fragility;
      const fb = b.profile.fragility;
      // A refused profile carries no claim to a position on this ladder, and sorting
      // it by a fallback of 0 or -1 would smuggle a claim back in (0 reads as "least
      // fragile", -1 as an emphatic version of the same lie this file exists to stop
      // telling). Every readable roster outranks every refused one regardless of
      // either one's raw number; refused rosters keep a total order among themselves
      // via the same tie-breaks.
      if (fa == null && fb == null)
        return b.raw - a.raw || a.profile.rosterId - b.profile.rosterId;
      if (fa == null) return 1;
      if (fb == null) return -1;
      return fb - fa || b.raw - a.raw || a.profile.rosterId - b.profile.rosterId;
    })
    .map((s) => s.profile);
}
/**
...
function buildProfile(
  ...
  if (assets.length < MIN_ASSETS_FOR_FRAGILITY)
    return refusedProfile(roster.rosterId, teamName, ownerName, assets, slots, replacement);
...
/**
 * The refusal register's answer for a roster this file cannot score - too few valued
 * assets to say anything, or none at all.
 *
 * THIS USED TO BE `emptyProfile`, and it fabricated `fragility: 0, band: "resilient"`
 * for a roster holding no priced asset at all - the exact mirror of the bug D102 fixed
 * in TCI (`getTimelineProfile` returning the literal word "straddling", the worst
 * posture, for the same condition). Zero and "resilient" is not a neutral floor here
 * the way TCI's `tci: 0` arguably was: paired with "resilient" it reads as "the
 * sturdiest roster in the league," which is the opposite claim to the one a roster the
 * app cannot read at all is entitled to make. `MIN_ASSETS_FOR_FRAGILITY` widened the
 * same failure to a second, mechanical case - see that constant's own comment - so one
 * function now answers for both rather than the zero-asset case alone.
 *
 * Not every field goes null. `startableValue`, `playerValue` and
 * `depthBeyondStarters` are not STATISTICS that need a sample to mean something - they
 * are sums and a solved lineup, true at any n including zero - so they stay real
 * numbers. What goes null is exactly the set of figures this file's own measurement
 * showed are an artefact of the count below `MIN_ASSETS_FOR_FRAGILITY`: the index, its
 * three components, the shares and HHI underneath them, the percentile and band they
 * would have produced, and the single point of failure (the same "top three is the
 * whole roster" problem, one player wide).
 */
function refusedProfile(rosterId, teamName, ownerName, assets, slots, replacement) {
  const assetCount = assets.length;
  const startable = startableValue(assets, slots);
  const playerValue = assets.reduce((s, a) => s + a.value, 0);
  const depth = depthBeyondStarters(assets, slots, replacement);
  const code = assetCount === 0 ? "NO_RECORD" : "INSUFFICIENT_SAMPLE";
  const because =
    code === "NO_RECORD"
      ? "No asset on this roster carries a price, so there is nothing to score fragility from."
      : `Read from ${assetCount} valued asset${assetCount === 1 ? "" : "s"} against a floor of ` +
        `${MIN_ASSETS_FOR_FRAGILITY}: at or below ${LOO_TOP_K} assets, "your three most ` +
        `load-bearing players" is the whole roster by construction, not a fraction of it - every ` +
        `real fixture-league roster, stripped to its own top ${LOO_TOP_K} assets, reads its LOO ` +
        `and concentration components (0.8 of the index) pinned to their ceiling regardless of ` +
        `which assets they are, so 80% of the figure would be an artefact of the count rather ` +
        `than a reading of it.`;
  const r = refusal(code, because);
  return {
    rosterId,
    teamName,
    ownerName,
    ...
    exposureScore: null,
    spofDamageShare: null,
    topKDamageShare: null,
    normalizedHhi: null,
    rawExposure: null,
    startableValue: startable,
    playerValue,
