...
function roster(values: number[]): FragilityAsset[] {
...
describe("concentration (normalized HHI)", () => {
  it("is 0 for a perfectly even set, at any size", () => {
    ...
    expect(concentration(Array.from({ length: 20 }, () => 7))).toBeCloseTo(0, 10);
  });
  ...
  it("scores a concentrated set above an even one at identical total", () => {
    ...
    expect(even.reduce((s, v) => s + v, 0)).toBe(conc.reduce((s, v) => s + v, 0));
  ...
  it("still maps an evenly filled benchmark roster to 0", () => {
    ...
    expect(concentration(even, concentrationBenchmark(SLOTS))).toBeCloseTo(0, 10);
  });
});
...
describe("the index", () => {
  ...
  it("becomes more fragile as load-bearing depth is stripped away", () => {
    ...
    for (let keep = bench.length; keep >= 0; keep--) {
      const raw = scoreFragility([...starters, ...bench.slice(0, keep)], five).raw;
  ...
  it("charges nothing for depth that was already redundant", () => {
    ...
    const two = [...starters, asset("b0", 1500, ["PG"]), asset("b1", 1500, ["PG"])];
  ...
  it("is the declared weighted combination of its components", () => {
    const s = scoreFragility(roster([4000, 3000, 2000, 1500, 1000, 800, 600, 400]), SLOTS);
    const expected =
      W_LOO * s.looScore +
...
describe("Roster Fragility Index over the league", () => {
  ...
  it("discriminates across the league instead of clustering", () => {
    ...
    for (const key of ["looScore", "concentrationScore", "exposureScore"] as const) {
  ...
  it("agrees between the single-roster getter and the league pass", () => {
    ...
    for (const p of all) {
      ...
      expect(one.singlePointOfFailure!.playerId).toBe(p.singlePointOfFailure!.playerId);
    }
  });
  ...
  it("is deterministic: same corpus in, identical numbers out", () => {
    ...
    expect(a.map((p) => p.exposureScore)).toEqual(b.map((p) => p.exposureScore));
