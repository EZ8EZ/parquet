...
describe("windowShort", () => {
  ...
  it("is the CODE for a refused one, never a dash", () => {
    ...
    expect(split).toBe("SPLIT_ROSTER");
    expect(none).toBe("INSUFFICIENT_SAMPLE");
...
describe("windowRefusalSummary", () => {
  ...
  it("groups by code, counts each, and says nothing is still arriving", () => {
    ...
    expect(s).toContain("SPLIT_ROSTER: 2 rosters are drawn as two ends");
    expect(s).toContain("INSUFFICIENT_SAMPLE: One roster holds");
  ...
  it("keeps register order however the rows are shuffled", () => {
    ...
    expect(a).toBe(b);
    expect(a.indexOf("INSUFFICIENT_SAMPLE")).toBeLessThan(
      a.indexOf("SPLIT_ROSTER"),
    );
  });
  it("grades nobody (D6)", () => {
    const s = windowRefusalSummary([
...
describe("windowSynthesis speaks in codes too", () => {
  ...
  it("opens with the viewer's own code and keeps the withheld season", () => {
    ...
    expect(s.startsWith("SPLIT_ROSTER: ")).toBe(true);
  ...
  it("distinguishes an empty record from a thin one, which one word could not", () => {
    ...
    expect(empty.startsWith("NO_RECORD: ")).toBe(true);
  });
});
