 ...
 * same as good. A torn-down roster can score low for exactly the same reason a healthy
 * one does - nothing left on it is load-bearing enough to lose - and the MOST torn-down
 * rosters do not score at all: `lib/metrics/fragility.js` refuses to publish a fragility
 * score/band below four valued assets, so this board never plots one at a fabricated
 * zero (D113). A chart that painted the low-RFI end green would be stating something
 * false even about the rosters it does plot.
 *
 ...
 */
export function assignQuadrant(tci, fragility, tciMid, fragilityMid) {
 ...
 */
export const TCI_BANDS = [
  {
    step: 1,
    max: 55,
    range: "under 55",
    meaning: "straddling - below the coherence floor",
  },
 ...
 */
export function axisDomain(values, opts) {
 ...
 */
export function placeLabels(points, opts) {
 ...
 */
export function buildQuadrantView(timelines, fragility, meRosterId) {
  const byRoster = new Map(fragility.map((f) => [f.rosterId, f]));
  // A REFUSED FRAGILITY READ IS DROPPED THE SAME WAY AN UNSCORED ROSTER ALREADY WAS.
  // This board already had that rule ("drops rosters the fragility pass did not score
  // rather than plotting them at zero") for a roster with no entry in `fragility` at
  // all; `lib/metrics/fragility.js` now hands back an entry for EVERY roster but
  // leaves `.fragility` null for one it refuses to score (too few valued assets, or
  // none), and the same rule applies for the same reason - a null is not a coordinate,
  // and plotting it at 0 would be exactly the fabrication the refusal exists to avoid.
  const joined = timelines.filter(
    (t) => byRoster.get(t.rosterId)?.fragility != null,
  );
  const tciMid = median(joined.map((t) => t.tci));
  const fragilityMid = median(
    joined.map((t) => byRoster.get(t.rosterId).fragility),
  );
  const points = joined
    .map((t) => {
