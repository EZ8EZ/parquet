...
import { TIMELINE_AXIS } from "./axes.js";
...
/**
 * The league's postures as counts, in `TIMELINE_AXIS` order.
 *
 * EXISTS SO A CENSUS CANNOT DISAGREE WITH A BOARD. /league used to count core-age bands
 * in its three tiles and print postures on its rows, which is how a page came to say
 * "3 REBUILDING" above a board that printed the word "rebuilding" against four rosters.
 * The tiles now read this, off the same `leagueTimelines` array the rows read, so the
 * two agree by construction rather than by coincidence.
 *
 * @param {{ posture: string }[]} timelines
 * @returns {{ posture: string, count: number }[]}
 */
export function postureCensus(timelines) {
  return TIMELINE_AXIS.words.map((posture) => ({
    posture,
    count: timelines.filter((t) => t.posture === posture).length,
  }));
}
