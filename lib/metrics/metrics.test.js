...
describe("Timeline Coherence Index", () => {
  ...
  it("carries a timelineBreak field (one or two named assets, or null) for every real roster", () => {
    // D114: the real league's timeline breaks are a mix now - some rosters still name
    // one asset, most now name a correlated pair. Both shapes must be well-formed.
    let sawPair = false;
    for (const r of h.rosters) {
      const p = getTimelineProfile(h, r.rosterId);
      if (p.timelineBreak === null) continue;
      expect(p.timelineBreak.delta).toBeGreaterThanOrEqual(1);
      expect(p.timelineBreak.assets.length === 1 || p.timelineBreak.assets.length === 2).toBe(
        true,
      );
      if (p.timelineBreak.assets.length === 2) sawPair = true;
      for (const a of p.timelineBreak.assets)
        expect(typeof a.label === "string" || a.label === null).toBe(true);
      // "does" for one asset, "do" for a pair - the shared substring covers both.
      expect(p.read).toContain("not fit that story");
    }
    // The whole point of this feature: at least one real roster's true best removal
    // IS a pair, or every test above would pass on a build that never found one.
...
describe("findTimelineBreak", () => {
  ...
  it("names the one asset dragging an otherwise-coherent core off its plan, when no pair beats it by enough to matter", () => {
    // A young core clustered near duration 4.8, plus one large-value asset sitting
    // alone at duration 2. Removing "Core Three" alongside the outlier does improve
    // TCI a little further (a real pair exists here too), but only by +2 - below
    // PAIR_MARGIN_MIN (+3) - so this stays a one-asset finding rather than a pair.
    const assets = [
    ...
    expect(b.assets[0].id).toBe("outlier");
    expect(b.delta).toBeGreaterThan(0);
    expect(breakAssetNames(b)).toBe("Outlier");
  });
  it("ignores a low-value asset when its distance is not extreme enough to matter", () => {
    ...
    // No single asset clears the floor here, so the pair sweep never even runs - a
    // pair can only REFINE an existing finding into two names, never invent one.
    const assets = [
  ...
  it("but a low-value asset far enough out can still clear the floor - product, not distance alone, decides", () => {
    ...
    expect(b).not.toBeNull();
    expect(b.assets).toHaveLength(1);
    expect(b.assets[0].id).toBe("farpick");
  });
  it("floors materiality at BREAK_MIN_DELTA rather than naming a trivial 0-1 point gain", () => {
    ...
    // dispersion by rounding-level noise, never a full point. A pair fares no
    // better (it is never even tried, since no single clears the floor first).
    const assets = [
  ...
  it("breaks a single-asset value tie on larger value, then on id, deterministically", () => {
    // Three assets - too few for the pair sweep to run at all (it needs >= 4) - so
    // this isolates the SINGLE search's own tie-break from any pair interaction.
    ...
    const assets = [
      ...
      { id: "mid", label: "Mid", value: 3000, duration: 4 },
    ];
    const tci = coherenceOf(assets).tci;
    const b1 = findTimelineBreak(assets, tci);
    const b2 = findTimelineBreak([...assets].reverse(), tci);
    expect(b1).not.toBeNull();
    expect(b1.assets).toHaveLength(1);
    // Order-independent: reversing the input array cannot change the winner.
    expect(b1.assets[0].id).toBe(b2.assets[0].id);
  });
  it("names a correlated PAIR, order-independently, when removing both together clears the single by enough", () => {
    // The fourth asset (mid2) makes the pair sweep eligible (n >= 4) and, once
    // z-high and a-low are both gone, leaves two identical mid assets - dispersion
    // 0, tci 100 - a bigger jump than either alone buys, well past PAIR_MARGIN_MIN.
    const assets = [
      { id: "z-high", label: "Z High", value: 1000, duration: 8 },
      { id: "a-low", label: "A Low", value: 1000, duration: 0 },
    ...
    const b2 = findTimelineBreak([...assets].reverse(), tci);
    expect(b1).not.toBeNull();
    expect(b1.assets).toHaveLength(2);
    const idsOf = (b) => new Set(b.assets.map((a) => a.id));
    expect(idsOf(b1)).toEqual(new Set(["z-high", "a-low"]));
    // Order-independent: reversing the input array cannot change which pair, or
    // change the reported delta.
    expect(idsOf(b1)).toEqual(idsOf(b2));
    expect(b1.delta).toBe(b2.delta);
    // Ordered by id string ("a-low" < "z-high"), not by scan order - see
    // `findTimelineBreak`'s own comment on why the pair's order cannot be allowed to
    // depend on which asset the caller happened to list first.
    expect(breakAssetNames(b1)).toBe("A Low and Z High");
  });
  it("stays a single asset when a pair exists but does not beat it by PAIR_MARGIN_MIN", () => {
    // A fourth asset placed exactly at the mean adds no removable dispersion of its
    // own, so pairing it with the single-best removal cannot buy the +3 required.
    const assets = [
      { id: "core1", label: "Core One", value: 5000, duration: 4 },
      { id: "core2", label: "Core Two", value: 5000, duration: 4 },
      { id: "outlier", label: "Outlier", value: 2000, duration: 9 },
      { id: "atmean", label: "At Mean", value: 10, duration: 4.4 },
    ];
    const tci = coherenceOf(assets).tci;
    const b = findTimelineBreak(assets, tci);
    expect(b).not.toBeNull();
...
describe("breakAssetNames", () => {
  it("is null for no break", () => {
    expect(breakAssetNames(null)).toBeNull();
  });
  it("joins a two-asset break with 'and', a one-asset break with no joiner", () => {
    expect(
      breakAssetNames({ assets: [{ label: "LeBron James" }], delta: 5 }),
    ).toBe("LeBron James");
    expect(
      breakAssetNames({
        assets: [{ label: "DeMar DeRozan" }, { label: "LeBron James" }],
        delta: 11,
      }),
    ).toBe("DeMar DeRozan and LeBron James");
  });
  it("is null rather than a broken string when an asset carries no label", () => {
    expect(
      breakAssetNames({ assets: [{ label: null }, { label: "LeBron James" }], delta: 5 }),
    ).toBeNull();
  });
});
describe("coherenceOf", () => {
  ...
  /*
   * THE DRAWN BAND IS THE PUBLISHED NUMBER.
   *
   * `DurationStrips` (components/charts.jsx) draws a ±1σ band at `rosterDuration ±
   * dispersion` behind each strip's dots, and the whole claim of that drawing is that
   * the band narrowing between "today" and "after" IS the TCI rising - not a decorative
   * correlate of it. That holds only because TCI is literally `1 - σ/SIGMA_REF`, so the
   * band's width in seasons is `2·SIGMA_REF·(1 - TCI/100)`.
   *
   * Pinned against real rosters and against synthetic bags on both sides of the clamp,
   * because it is an identity a future recalibration could silently break: change
   * SIGMA_REF and the drawing stays valid, change the TCI formula to anything that is
   * not a linear function of dispersion and it does not.
   */
  it("has a ±1σ band width that IS the TCI, in seasons", () => {
    const bags = [
      ...h.rosters.map((r) => getTimelineProfile(h, r.rosterId).assets),
      [
        { value: 100, duration: 4 },
        { value: 100, duration: 4.2 },
      ],
      [
        { value: 900, duration: 1.9 },
        { value: 100, duration: 7.2 },
      ],
      [{ value: 50, duration: 3 }],
    ];
    for (const assets of bags) {
      const c = coherenceOf(assets);
      if (c.totalValue === 0) continue;
      const bandWidth = 2 * c.dispersion;
      if (c.dispersion >= SIGMA_REF) {
        // Above the clamp the number is pinned at 0 and the band keeps widening; the
        // chart's caption prints both figures for exactly this case.
        expect(c.tci).toBe(0);
        continue;
      }
      // Allow half a TCI point, which is `Math.round`'s own resolution.
      expect(bandWidth).toBeCloseTo(2 * SIGMA_REF * (1 - c.tci / 100), 1);
    }
  });
