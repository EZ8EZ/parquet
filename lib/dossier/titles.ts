 ...
 */
import { titlesByOwner } from "../playoffs";
...
export interface TitleSummary {
  ...
  seasons: string[];
  /** "2024 champion" for one, "3x champion (2022, 2024, 2025)" for more. */
  label: string;
}

/** `null` for an empty list - there is no zero-title summary, only no summary. */
export function titleSummary(seasonsWon: readonly string[]): TitleSummary | null {
 ...
 */
export function titleSummariesByOwner(
  h: LeagueHistory,
  principals: PrincipalIndex,
): Map<string, TitleSummary> {
