...
function isotonicDecreasing(points) {
  ...
  while (i < blocks.length - 1) {
    if (blocks[i].value < blocks[i + 1].value - 1e-12) {
      ...
      blocks.splice(i, 2, {
        value:
          (a.value * a.weight + b.value * b.weight) / (a.weight + b.weight),
...
async function main() {
  ...
  console.log(
    `scoring (all ${h.chain.length} seasons): ${JSON.stringify(scoring)}`,
  );
  const players = await getJson(`${STATS_BASE}/v1/players/nba`);
  const seasons = [];
  ...
  for (const season of seasons) {
    const blob = await getJson(`${STATS_BASE}/v1/stats/nba/regular/${season}`);
    const rows = [];
