...
async function main() {
  ...
  const origin = process.argv[2] ?? process.env.PARQUET_ORIGIN ?? "http://localhost:3000";
  ...
  const managers = rosters
    ...
    .map((r) => ({
      ...
      displayName: byId.get(r.ownerId as string)?.displayName ?? (r.ownerId as string),
      teamName: byId.get(r.ownerId as string)?.teamName ?? null,
    }))
