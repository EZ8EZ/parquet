...
_Prepared 2026-07-28. Sources are public marketing pages, free tool surfaces, GitHub repos, app-store listings, and third-party reviews. No proprietary or paid datasets were scraped. Several fantasy sites (the category-planning site, the dynasty-focused host platform, a subscription editorial site) return HTTP 403 to automated fetching; those details are corroborated from search-indexed snippets and reputable third-party reviews and are flagged inline._

## 1. Executive summary

The dynasty-basketball tooling market is **fragmented across three non-overlapping camps** - content/editorial (a subscription editorial site, an SEO-led editorial site), league platforms (the dynasty-focused host platform, the mainstream platform), and analytics engines (the projection engine, the category-planning site) - and **no single product unifies crowdsourced trade values, pick/asset modeling, and league sync** the way dynasty football's ecosystem has matured. Our original hypothesis that "no the leading dynasty-football crowd-value site-for-NBA exists" is **REFUTED**: **the leading NBA crowd-value entrant** (that NBA crowd-value entrant) is a genuine crowdsourced dynasty-basketball value site with an ELO engine, and there are several hybrid crowd/expert entrants (a hybrid NBA value site, another hybrid NBA entrant, the category-planning site's blended calculator) plus an open-source clone (an open-source NBA clone). **However, none of them has the leading dynasty-football crowd-value site-scale vote liquidity** - the category has early incumbents but no entrenched leader, and the the executed-trades value site "values from real executed trades" model is **completely unoccupied for the NBA**. The strategic implication: this is not greenfield, but it is winnable, and - critically for us - none of these competitors touches the behavioral/decision-intelligence layer where our four differentiators (decision ledger, revealed-vs-stated strategy, manager dossiers, adversarial LLM analyst) would live.
...
#### the category-planning site (a category-league planning site)
1. **Does well:** Best-in-class category-league planning, anchored by the Advanced NBA Schedule Grid (which teams play most games in a week - the streaming bible for H2H). Broad free access; imports the mainstream platform, ESPN, the dynasty-focused host platform, Sleeper.
...
3. **Pricing:** its premium tier **$2.50/mo** - one sub covers all four sister sites (NBA/MLB/NHL/NFL); much is usable free. (Confirmed via search + Patreon.)
...
#### the projection engine (a projection-engine site)
...
2. **IA / nav:** Tools (Player Rankings, Team Analysis, Trade Analysis, Lineups, Box Scores, Schedule Grid); Members (Projections, Draft Tracker, Projected Standings, Smart Tool, Advanced Ownership, Depth Charts, Tiers); the "Monster" suite (Draft/Analysis/**Trade**/Matchup/Usage Monster); dynasty via a proprietary dynasty ranking set.
...
#### the dynasty-focused host platform - Dynasty Basketball (that host platform)
...
2. **IA / nav:** Free game (Create/Join, Rankings, Analysis, My Team, Players, Matchups). **its paid tier** adds ~19 tools: Research Assistant, **Trade Hub** (most-traded, team analysis, **top-3 trade partners**), Draft Tools, Expert Rankings (the mainstream platform + a subscription editorial site).
...
#### a subscription editorial site - Basketball Dynasty (that editorial site)
...
3. **Pricing:** Subscription (season/monthly); NBA price not exposed to fetch; also powers the mainstream platform's expert rankings.
...
#### an SEO-led editorial site - Basketball Dynasty (that SEO-led site)
...
#### the leading dynasty-football crowd-value site (that crowd-value site) - the crowdsourced mechanic
...
#### the executed-trades value site (that executed-trades site) - values from real executed trades
...
5. **Pattern most worth stealing:** **Revealed-preference valuation from executed trades**, paired with a **public values API** as a distribution wedge. (Contrast worth internalizing: the crowd-value leader = _stated_ preference; the executed-trades value site = _revealed_ preference. The best product fuses both.)

#### the open-source aggregator (that aggregator) - open source
1. **Does well:** All-in-one **aggregator** - its explicit purpose is to "eliminate toggling between multiple websites." It doesn't generate its own values; it ingests the major value sources and ties them to your live league rosters, then layers analytics. Broadest platform support (ESPN, the mainstream platform, Sleeper, another platform, another host, the dynasty-focused host platform, another host). Stack (public GitHub, source-available; confirm license): Angular + Node/Express + PostgreSQL + a Python daily-scrape cron.
...
#### the open-data project (that open-data project) - open data + published methodology
1. **Does well:** Radical transparency and reusable infrastructure - the "developer/analyst layer" of dynasty. Publishes open CSVs including **`a cross-platform player-id crosswalk`, a cross-platform player-ID crosswalk** (maps IDs across Sleeper/another platform/ESPN - quietly the most valuable asset here), an expert-consensus source ECR, and player/pick `values.csv`. **Transparent value formula:** `Value = 10500 * e^(FP_ECR * -0.0235)` with a user-tunable elite-vs-depth curve. Rookie-pick values from blended GAM models with **future picks discounted to ~80%** (present-value logic). Ships R packages (`ffopportunity`, `ffsimulator`).
...
### 2C. Direct competitor (a dynasty FOOTBALL companion; we build the basketball analogue)

_A design-system-review prototype deployment. No pricing shown. Inspected live at a 375x812 mobile viewport across all four routes. Kept anonymous here deliberately; the value is the UX lessons, not the name._
...
|---|---|---|---|---|---|---|---|---|---|---|
| **the category-planning site** | ◐ (blended) | ○ | ● | ● | ◐ | ● | ○ | ○ | ○ | $2.50/mo |
| **the projection engine** | ○ | ○ | ● (Trade Monster) | ◐ (DURANT) | ◐ | ● | ○ | ○ | ○ | ~$10-30/mo |
| **the dynasty-focused host platform** | ○ | ○ | ● (league) | ○ | ● (picks 10yr, multi-team) | n/a (is the league) | ○ | ● (~4.8★) | ○ | Free / ~$80-130/season |
| **the mainstream platform Fantasy NBA** | ○ | ○ | ● | ◐ | ○ | n/a (is the league) | ● (top-3 partners) | ● (benchmark) | ○ | Free / $2.92/mo (Fantasy+) |
| **a subscription editorial site NBA** | ○ | ○ | ● | ◐ (editorial) | ◐ | ● | ○ | ○ | ○ | Subscription |
| **an SEO-led editorial site NBA** | ○ | ○ | ◐ | ◐ (editorial) | ● (rookie boards) | ○ | ○ | ○ | ○ | Freemium |
| **the leading NBA crowd-value entrant (NBA)** | ● (Best/Mid/Worst → ELO) | ○ | ● | ● | ● (picks) | ◐ (new) | ○ | ○ | ○ | Free (not disclosed) |
| **a hybrid NBA value site (NBA)** | ◐ (10% crowd ELO) | ○ | ● | ● | ● | ● | ○ | ○ | ○ | Free/freemium |
| **the leading dynasty-football crowd-value site (NFL)** | ● (Keep/Trade/Cut → ELO) | ○ | ● | ● | ● | ● | ○ | ○ | ○ | Free |
| **the executed-trades value site (NFL)** | ○ | ● | ● | ● | ● | ● | ○ | ○ | ● (API) | Free |
| **the open-source aggregator (NFL)** | ◐ (ingests the crowd-value leader) | ◐ (ingests) | ● | ● | ● | ● (7 platforms) | ● (Trade Finder) | ○ | ● (open source) | Free / $6/mo |
| **the open-data project (NFL)** | ○ | ○ | ● | ● | ● (PV-discounted) | ◐ | ○ | ○ | ● (open data + crosswalk) | Free |
| **Sleeper (NFL/NBA)** | ○ | ○ | ● (in-league) | ○ | ● | n/a (is the league) | ◐ (two-way block) | ● (best-in-class) | ● (public API) | Free |
| **Direct competitor (NFL)** | ◐ (fuses 3 sources) | ◐ (fuses) | ● | ● (tier/spread/range) | ● | ● (Sleeper) | ◐ (rival scouting) | ○ (web/PWA) | ○ | n/a (prototype) |
...
**Verdict: Hypothesis REFUTED - a the leading dynasty-football crowd-value site-for-NBA does exist (several, in fact), BUT none has leader-scale liquidity, and the the executed-trades value site "real executed trades" model is entirely unoccupied for the NBA.**

**Tier 1 - true crowd-vote-style crowdsourced NBA value sites (these refute the hypothesis):**
- **the leading NBA crowd-value entrant (that NBA crowd-value entrant)** - the clearest the crowd-value leader-for-NBA. Users "rank them: Best, Mid, or Worst based on general dynasty league settings" (a ternary comparative vote - the crowd-value leader's mechanic generalized). "Your votes combine with the community to establish player values." Runs an **ELO system** ("Biggest ELO changes over the last 30 days"), has a **Trade Calculator** (players + future picks), Rankings, League Import, and a Trade Database; dynasty-basketball exclusive. Referenced by the "Know Ball Dynasty" podcast and used in r/dynastybball. **Caveat:** the homepage "Data Points Collected" counter rendered as **"0"** on direct fetch - strongly implying low maturity / thin vote volume, not leader-scale.
- **an open-source NBA clone** (GitHub: `a single-maintainer repo`) - an explicit open-source the crowd-value leader clone: "rankings are fully crowdsourced," with a trade calculator and a "**Keep/Trade/Cut game**." Clearly hobbyist/early (single maintainer, ~0 stars).

**Tier 2 - hybrids with a real crowd-voting component (partial equivalents):**
- **a hybrid NBA value site (that hybrid NBA site)** - most sophisticated; values "blend **90% expert consensus with 10% crowd ELO**," K-means tiers, daily refresh, league import, and a "Hype Gap" (crowd sentiment vs analytics). Real crowd voting exists but is a minority input. (Direct fetch returned HTTP 502 at time of writing; existence/methodology corroborated via search.)
- **another hybrid NBA entrant (that entrant)** - "Expert rankings meet community sentiment - all in one number," with a real-time trade calculator. Methodology not fully disclosed.
- **the category-planning site's dynasty trade calculator** - blends its own dynasty rankings with "crowd-sourced trade values" reflecting "actual dynasty league behavior."

**What does NOT exist / does NOT qualify:**
- **the executed-trades value site has no NBA product** - football-only. The revealed-preference "values from real executed trades" approach is a **wide-open lane for the NBA**.
- **the leading dynasty-football crowd-value site itself appears football-only** - all live URLs are NFL; claims that "the crowd-value leader added basketball" trace only to low-quality SEO blogs and could not be confirmed on that crowd-value site. Treat "the crowd-value leader does NBA" as false/unverified.
- **the open-data project, an expert-consensus source, the projection engine, a subscription editorial site, an SEO-led editorial site** for basketball are editorial/expert-consensus or projection-model - **not head-to-head crowdsourcing**, so they don't qualify.

**Strategic read:** The category is **not empty, but immature and fragmented, with no entrenched high-liquidity leader** the way the crowd-value leader dominates dynasty football. Open lanes a new entrant can own: (1) a **real-executed-trades valuation model** (nobody does the executed-trades value site-for-NBA); (2) **achieving trustworthy vote/trade liquidity** (every current NBA crowd tool appears thin); (3) deep **league-platform import integrations**; and above all (4) the **behavioral/decision-intelligence layer none of them touch.** Building our own valuation model is still defensible - but we should **treat the leading NBA crowd-value entrant and a hybrid NBA value site as live benchmarks to beat, not as proof of a category we're inventing.** Recommended next step: a hands-on audit of the leading NBA crowd-value entrant's and a hybrid NBA value site's actual value quality and vote volume before finalizing our valuation approach.

_Evidence caveat: most of these are JS SPAs that partially blocked automated fetching, and Reddit was not directly fetchable, so the **scale/maturity** reads are inferred from snippets and secondary references; the **existence and methodology** findings are solid and cross-confirmed by two independent research passes plus direct fetches of the leading NBA crowd-value entrant and searches surfacing all named tools._
...
1. **Sleeper + the dynasty-focused host platform league import & sync** (transaction history is the fuel for every differentiator; the dynasty-focused host platform is where dynasty NBA lives, Sleeper is the UX benchmark). Build/own an **NBA cross-platform player-ID crosswalk** first (steal from the open-data project's playbook).
2. **Roster view** - mobile-first, stacked cards (never clipped tables; explicitly avoid the competitor's Rankings mistake).
3. **Asset values + dynasty rankings** - a transparent, tunable baseline value model (start expert/projection-derived with a user elbow of an **elite-vs-depth curve slider**, à la the open-data project) so we're not blocked on crowd liquidity. Show tier + range.
4. **Trade evaluator** - Side A/Side B stacked, edit-in-place, with a **consolidation premium** (stars > quantity), mirroring the competitor's grading logic.
...
8. **Manager Dossiers** - behavioral profiles of leaguemates from public league transaction history (trade cadence, positional tendencies, over/under-pays, contender/rebuilder). Naturally pairs with a **trade-partner matcher** (steal the mainstream platform's "top-3 partners" + Sleeper's two-way trade block as the UX).

**Tier 2 - Engagement & moat (post-v1)**
9. **Crowdsourced value voting (crowd-vote-style Keep/Trade/Cut → ELO)** with contribute-to-consume - deferred deliberately (see §6): only worth it once we have enough users to reach liquidity the leading NBA crowd-value entrant lacks. Until then, our values are model-derived.
10. **Rookie/draft-pick board** with present-value discounting for future picks (cheap engagement magnet; steal an SEO-led editorial site + the open-data project).
11. **Playoff/standings Monte-Carlo simulator** (parity with the open-source aggregator/the competitor; not a differentiator).

**Non-negotiable UX rules for v1 (lessons from the competitor):** real fixed bottom tab bar with icon + label (Home / Roster / Trade / League / Analyst); **no floating overlays that occlude content**; rankings as stacked cards, never clipped tables; every player card is a trade/ledger entry point (Sleeper pattern).

---

## 6. What we are deliberately NOT building (and why)

1. **Not a league-hosting platform.** the dynasty-focused host platform and Sleeper own hosting/transactions and have a decade head start; we integrate with them, we don't replace them. We're a companion.
2. **Not a crowdsourced value engine at launch.** A crowd vote is worthless without liquidity - the leading NBA crowd-value entrant shows "0 data points" and every NBA crowd tool is thin. We ship a transparent model-derived value instead, and only add crowd-vote-style voting once our user base can actually reach trustworthy volume. Chasing it early would burn effort on a cold-start we can't win yet.
3. **Not a the executed-trades value site-style real-executed-trades model in v1.** It's the most attractive _empty_ lane, but it needs large volumes of ingested completed NBA trades to be credible - a chicken-and-egg we can't solve at launch. Flag it as a v2 moat once league-sync volume exists.
4. **Not projection/DFS/streaming tooling.** the projection engine and the category-planning site own daily projections, schedule grids, and lineup optimizers. That's a different (redraft/daily) audience and a feature arms race we'd lose. Dynasty decision-making is our lane.
5. **Not editorial content / rankings articles.** a subscription editorial site and an SEO-led editorial site own SEO editorial. We are a tool, not a publisher; our "content" is generated from the user's own data by the LLM analyst.
6. **Not a native iOS/Android app in v1.** Mobile-first **PWA** only - faster iteration, one codebase, installable. Native can follow if retention justifies it; the dynasty-focused host platform's dated native app shows native ≠ good.
7. **Not multi-sport at launch.** the category-planning site's four-site bundle is a distraction pattern for us. Win dynasty _basketball_ (the underserved, un-consolidated market) before diluting focus.
8. **Not a black-box valuation.** We publish our methodology and expose a tunable value curve (the open-data project-style). Transparency is cheap trust and a wedge against opaque competitors - building an unexplainable model would forfeit that.
